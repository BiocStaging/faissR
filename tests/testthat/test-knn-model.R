...
test_that("predict preserves nearest-neighbour route metadata", {
  ...
  y <- factor(c("a", "a", "b", "b"))
  fit <- knn(x, y, backend = "cpu", method = "exact", metric = "cosine", tuning = "off", k = 2L)

  pred <- predict(fit, x[1:2, , drop = FALSE], backend = "cpu", tuning = "off")
  proba <- predict(fit, x[1:2, , drop = FALSE], backend = "cpu", tuning = "off", type = "prob")

  pred_meta <- attr(pred, "faissR_nn")
  proba_meta <- attr(proba, "faissR_nn")
  expect_equal(pred_meta$requested_backend, "cpu")
  ...
  expect_equal(proba_meta$requested_backend, "cpu")
  ...
  reg <- knn(x, c(0, 0, 1, 1), backend = "cpu", method = "exact", task = "regression", tuning = "off", k = 2L)
  reg_pred <- predict(reg, x[1:2, , drop = FALSE], backend = "cpu", tuning = "off")
  reg_meta <- attr(reg_pred, "faissR_nn")
  expect_equal(reg_meta$requested_backend, "cpu")
