...
test_that("public high-level APIs use session-resolved backends and auto tuning", {
  ...
  for (name in names(wrappers)) {
    f <- wrappers[[name]]
    expect_null(eval(f$backend), info = name)
  ...
  expect_null(eval(predict_formals$backend))
...
test_that("float32 C-callable returns stable KNN metadata", {
  ...
  expect_equal(attr(out, "resolved_backend"), "faiss_flat_l2")
  expect_true(out$distance_is_metric)
  expect_identical(out$distance_semantics, "euclidean_distance")
  expect_true(out$distance_comparable_across_queries)
  expect_identical(out$distance_order, "smaller_is_better")
  expect_identical(attr(out, "distance_semantics"), "euclidean_distance")
...
test_that("auto metadata labels hardware extrapolation without changing selection", {
  route <- list(
    ...
    predicted_backend = "faiss_gpu_flat_l2",
    selected_backend = "faiss_gpu_flat_l2",
    calibration_hardware_profile_id = "uct_hpc_cpu12_nvidia_l40s_2026",
    calibration_gpu_model = "NVIDIA L40S",
    calibration_cpu_model = NA_character_
  )
  old <- options(faissR.runtime_gpu_model = "NVIDIA L40S")
  on.exit(options(old), add = TRUE)

  matched <- faissR:::nn_auto_hardware_metadata(route)
  expect_equal(matched$selected_backend, "faiss_gpu_flat_l2")
  expect_equal(matched$hardware_match_status, "matched")
  expect_equal(matched$hardware_evidence, "calibration_hardware_matched")
  expect_false(matched$hardware_conservative_fallback)

  options(faissR.runtime_gpu_model = "NVIDIA A100-SXM4-80GB")
  extrapolated <- faissR:::nn_auto_hardware_metadata(route)
  expect_equal(extrapolated$selected_backend, "faiss_gpu_flat_l2")
  expect_equal(extrapolated$hardware_match_status, "mismatch")
  expect_equal(extrapolated$hardware_evidence, "hardware_extrapolated_unvalidated")
  expect_equal(
    extrapolated$hardware_policy_action,
    "static_policy_retained_no_hardware_fallback"
  )
})
...
test_that("removed metrics are rejected by the public API", {
  x <- matrix(seq_len(24), nrow = 6)
  expect_error(
    nn(x, k = 2L, metric = "inner_product"),
