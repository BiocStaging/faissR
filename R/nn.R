...
with_faiss_gpu_runtime <- function(params, expr) {
  ...
  old <- Sys.getenv(
    c("FAISSR_FAISS_GPU_QUERY_BATCH_SIZE", "FAISSR_FAISS_GPU_REUSE_RESOURCES"),
    unset = NA_character_
  )
  on.exit({
...
cpu_flatlike_params <- function(n,
                                p,
                                k,
                                metric = "euclidean",
                                target_recall = 0.99,
  ...
  if (identical(method, "flat")) {
    return(cpu_flat_params(n, p, k, metric = metric, target_recall = target_recall))
  }
  if (identical(method, "bruteforce")) {
    return(cpu_bruteforce_params(n, p, k, metric = metric, target_recall = target_recall))
  }
  cpu_exact_params(n, p, k, metric = metric, target_recall = target_recall)
}

cuda_flatlike_params <- function(n,
                                 p,
                                 k,
                                 metric = "euclidean",
                                 target_recall = 0.99,
  ...
  method <- public_nn_method_label(requested_method %||% "exact")
  if (identical(method, "flat")) {
    return(cuda_flat_params(n, p, k, metric = metric, target_recall = target_recall))
  }
  if (identical(method, "bruteforce")) {
    return(cuda_bruteforce_params(n, p, k, metric = metric, target_recall = target_recall))
  }
...
attach_cpu_exact_tuning <- function(result, params, output, n_threads, extra = NULL) {
  ...
  backend_label <- params$result_backend %||% params$resolved_backend %||% "faiss_flat_l2"
  strategy <- switch(
    method,
  ...
  meta <- c(
    list(
      ...
      recommended_n_threads = as.integer(params$recommended_n_threads %||% NA_integer_),
      actual_output = output,
      recommended_output = params$recommended_output %||% NA_character_,
      faiss_query_batch_size = as.integer(params$faiss_query_batch_size %||% NA_integer_),
...
attach_cuda_exact_tuning <- function(result, params, output, n_threads, extra = NULL) {
  if (!is.list(params)) return(result)
  method <- params$tuning_method %||% "exact"
  backend_label <- params$result_backend %||% params$resolved_backend %||% "faiss_gpu_flat_l2"
  resolved_default <- if (identical(method, "bruteforce")) "cuda_cuvs_bruteforce" else "faiss_gpu_flat_l2"
  ...
  meta <- c(
    list(
      ...
      actual_output = output,
      recommended_output = params$recommended_output %||% NA_character_,
      faiss_gpu_query_batch_size = as.integer(params$faiss_gpu_query_batch_size %||% NA_integer_),
      faiss_gpu_reuse_resources = isTRUE(params$faiss_gpu_reuse_resources),
...
fitted_nn_index_result <- function(data,
  ...
  out <- tryCatch(with_faiss_query_batch_size(params, {
    if (identical(kind, "hnsw")) {
  ...
  if (identical(kind, "flat")) {
    ...
    result <- attach_cpu_exact_tuning(result, params, output, n_threads, cache_meta)
...
nn_compute <- function(data,
  ...
  if (isTRUE(data_float32) || isTRUE(points_float32)) {
    ...
    if (identical(metric, "inner_product") &&
        backend %in% c("faiss_gpu_flat", "faiss_gpu_flat_l2", "cuda_faiss_flat_l2")) {
    ...
    if (metric %in% c("cosine", "correlation")) {
      ...
      if (backend %in% c("faiss_ivf", "cpu_faiss_index_ivf", "faiss_ivf_flat")) {
        ...
        params <- faiss_ivf_params(data_dim[[1L]], k, metric = metric,
                                   p = data_dim[[2L]], target_recall = target_recall)
      ...
      if (identical(backend, "faiss_ivfpq")) {
        ...
        params <- faiss_ivf_params(data_dim[[1L]], k, metric = metric,
                                   p = data_dim[[2L]], method = "ivfpq",
                                   target_recall = target_recall)
        pq <- faiss_ivfpq_pq_params(data_dim[[2L]], n = data_dim[[1L]], ivf_params = params)
    ...
    if (backend %in% c("cuvs_bruteforce", "cuda_cuvs_bruteforce", "cuda_cuvs_exact")) {
      ...
      brute_params <- cuda_bruteforce_params(data_dim[[1L]], data_dim[[2L]], k,
                                             metric = metric,
                                             target_recall = target_recall)
      metric_inputs <- NULL
      search_data <- data
      ...
      brute_distance_output <- if (is.null(metric_inputs)) output else "double"
    ...
    if (backend %in% c("cuda_cuvs_cagra", "cuda_cagra", "gpu_cagra")) {
      ...
      params <- cuvs_cagra_params(data_dim[[1L]], k, p = data_dim[[2L]], metric = metric, target_recall = target_recall)
      ...
      attr(result, "approximation") <- list(
        ...
        input_type = out$input_type %||% if (isTRUE(use_float32_input)) "float32" else "double",
        ...
        transform_storage = metric_inputs$transform_storage %||% if (isTRUE(use_float32_input)) "float32" else "double",
        transform_cache = metric_inputs$transform_cache %||% NULL
      )
      result <- append_nn_tuning_metadata(result, params)
      return(result)
    ...
    if (identical(backend, "faiss_ivfpq_fastscan")) {
      ...
      validate_faiss_cpu_ivfpq_training_size(data_dim[[1L]])
      params <- ivfpq_fastscan_cpu_params(data_dim[[1L]], data_dim[[2L]], k, target_recall = target_recall, metric = metric)
      if (metric %in% c("cosine", "correlation")) {
    ...
    if (backend %in% c("cuvs_nndescent", "cuda_cuvs_nndescent", "cuda_nndescent")) {
      ...
      attr(result, "approximation") <- list(
        ...
        transform_storage = if (is.null(metric_inputs)) "float32" else (
          metric_inputs$transform_storage %||% "float32"
        ),
        transform_cache = if (is.null(metric_inputs)) NULL else (metric_inputs$transform_cache %||% NULL),
        input_type = "float32",
    ...
    if (backend %in% c("cpu_nsg", "cuda_nsg")) {
      ...
      attr(result, "approximation") <- c(
        list(
          ...
          tuning_target_recall_code = as.integer(params$tuning_target_recall_code %||% NA_integer_),
          tuning_benchmark_basis = params$tuning_benchmark_basis %||% NA_character_,
          tuning_benchmark_target_met = isTRUE(params$tuning_benchmark_target_met),
          ...
          requested_target_recall = as.numeric(params$requested_target_recall %||% NA_real_),
  ...
  if (identical(output, "float") &&
    ...
    exact_params <- cpu_flatlike_params(nrow(data), ncol(data), k, metric = metric,
                                        target_recall = target_recall,
                                        requested_method = requested_method)
    if (metric %in% c("euclidean", "inner_product")) {
      cached <- fitted_nn_index_result(
  ...
  if (backend %in% c("faiss", "cpu_faiss", "cpu_faiss_flat", "faiss_flat", "faiss_flat_l2")) {
    ...
    exact_params <- cpu_flatlike_params(nrow(data), ncol(data), k, metric = "euclidean",
                                        target_recall = target_recall,
                                        requested_method = requested_method)
    ...
    result_backend <- if (identical(backend, "faiss")) "faiss" else "faiss_flat_l2"
  ...
  if (identical(backend, "faiss_flat_ip")) {
    ...
    exact_params <- cpu_flatlike_params(nrow(data), ncol(data), k, metric = "inner_product",
                                        target_recall = target_recall,
                                        requested_method = requested_method)
    cached <- fitted_nn_index_result(
      data = data,
  ...
  if (backend %in% c("faiss_ivf", "cpu_faiss_index_ivf", "faiss_ivf_flat")) {
    ...
    params <- faiss_ivf_params(nrow(data), k, metric = metric,
                               p = ncol(data), target_recall = target_recall)
    if (metric %in% c("cosine", "correlation")) {
      return(faiss_ivf_normalized_metric_result(
        data = data,
        points = points,
  ...
  if (backend %in% c("cuvs_ivfpq", "cuda_cuvs_ivfpq", "cuvs_ivf_pq", "cuda_cuvs_ivf_pq")) {
    ...
    use_float32_output <- identical(output, "float") && is.null(metric_inputs)
    params <- faiss_ivf_params(nrow(data), k, metric = metric,
                               p = ncol(search_data), backend = "cuda",
                               method = "ivfpq", target_recall = target_recall)
  ...
  if (backend %in% c("cuvs_bruteforce", "cuda_cuvs_bruteforce", "cuda_cuvs_exact")) {
    ...
    brute_params <- if (metric %in% c("euclidean", "cosine", "correlation")) {
      cuda_bruteforce_params(nrow(data), ncol(data), k,
                             metric = metric,
                             target_recall = target_recall)
    } else {
      NULL
...
normalize_nn_method <- function(method) {
  ...
  if (method %in% names(style_aliases)) method <- unname(style_aliases[[method]])
  labels <- nn_method_labels()
  if (!method %in% labels) {
      stop(
        "`method` must be one of \"auto\", \"exact\", \"flat\", \"bruteforce\", ",
        "\"grid\", \"hnsw\", \"ivf\", \"ivfpq\", \"vamana_style\", ",
        "\"nsg_style\", \"nndescent_style\", \"ivfpq_fastscan\", or \"cagra\". ",
...
nn_method_labels <- function() {
  c(
    ...
    "hnsw", "ivf", "ivfpq", "vamana", "nsg", "nndescent",
...
cuvs_cagra_build_algo_for_shape <- function(n, p, k, self_query, params = NULL) {
  ...
  if (identical(requested, "auto")) {
    table_algo <- as.character(params$cagra_build_algo %||% NA_character_)[[1L]]
    if (!is.na(table_algo) && nzchar(table_algo) && !identical(table_algo, "auto")) {
      return(table_algo)
    }
  }
...
nn_capability_row <- function(method, backend, metric) {
  ...
  if (identical(backend, "auto")) {
    ...
    if (identical(backend, "cpu")) {
      ...
      notes <- "CPU exact search supports all public metrics through FAISS Flat routes, with cosine and correlation using normalized Flat IP internally."
    } else {
      if (euclidean) {
        ...
        notes <- "CUDA cosine and correlation exact/brute-force search can use FAISS GPU Flat metric-aware routes or direct cuVS brute force with exact metric transforms."
      }
    }
    ...
    notes <- if (identical(method, "ivf")) {
      "FAISS IVF-Flat supports Euclidean/L2; cosine/correlation use row transforms followed by IVF search."
    } else {
      "FAISS IVFPQ supports Euclidean/L2; cosine/correlation use row transforms followed by IVFPQ search."
    }
    ...
    notes <- if (!supported) {
      if (identical(backend, "cpu")) {
        "CPU IVFPQ FastScan supports Euclidean/L2, cosine, and correlation."
      } else {
        "CUDA IVFPQ FastScan supports Euclidean/L2, cosine, and correlation."
      }
    } else if (identical(backend, "cpu")) {
      "Uses FAISS IndexIVFPQFastScan with 4-bit PQ lookup tables and optional Flat reranking; cosine normalizes rows and correlation centers/normalizes rows before FastScan L2."
    } else {
      "Uses direct cuVS IVF-PQ with 4-bit compressed codes; cosine row-normalizes, correlation row-centers plus row-normalizes. The route does not silently fall back to CPU FAISS FastScan."
    }
    ...
    notes <- if (identical(backend, "cpu")) {
      "Native CPU NNDescent supports Euclidean/L2 self-KNN; cosine/correlation use normalized Euclidean graph search. Seed neighbours use random-projection windows plus deterministic row fill, with flat row-major graph buffers and fixed-width reverse-neighbour storage in C++."
    } else if (supported) {
...
resolve_public_nn_backend <- function(backend,
  ...
  if (identical(device, "cpu")) {
    ...
    if (identical(method, "ivfpq_fastscan") && !metric %in% c("euclidean", "cosine", "correlation", "inner_product")) {
      stop("CPU `method = \"ivfpq_fastscan\"` currently supports `metric = \"euclidean\"`, `\"cosine\"`, `\"correlation\"`, or `\"inner_product\"`.", call. = FALSE)
    }
    switch(
      method,
      exact = if (metric %in% c("euclidean", "cosine", "correlation", "inner_product") && isTRUE(faiss_available())) {
        switch(
          metric,
          inner_product = "faiss_flat_ip",
          cosine = "faiss_flat_cosine",
      ...
      bruteforce = if (metric %in% c("euclidean", "cosine", "correlation", "inner_product") && isTRUE(faiss_available())) {
        switch(
          metric,
          inner_product = "faiss_flat_ip",
          cosine = "faiss_flat_cosine",
...
public_nn_cpu_route_supported <- function(method, metric) {
  ...
  switch(
    ...
    ivfpq_fastscan = metric %in% c("euclidean", "cosine", "correlation", "inner_product"),
    cagra = FALSE,
    FALSE
  )
}
...
nn_runtime_cpu_model <- function() {
  ...
  if (exists("cpu_model", envir = .faissR_auto_hardware_cache, inherits = FALSE)) {
  ...
  if (length(value) != 1L || is.na(value) || !nzchar(value)) value <- NA_character_
...
nn_runtime_gpu_model <- function() {
  ...
  override <- as.character(override)[1L]
  if (!is.na(override) && nzchar(override)) return(override)
  if (exists("gpu_model", envir = .faissR_auto_hardware_cache, inherits = FALSE)) {
    return(.faissR_auto_hardware_cache$gpu_model)
  }
  value <- tryCatch(cuda_native_summary()$device, error = function(e) NA_character_)
  value <- as.character(value)[1L]
  if (is.na(value) || !nzchar(value)) value <- NA_character_
...
nn_auto_hardware_metadata <- function(route) {
  ...
  route$hardware_policy_action <- "static_policy_retained_no_hardware_fallback"
  route$hardware_conservative_fallback <- FALSE
  route$hardware_evidence_note <- if (identical(match_status, "matched")) {
    "Runtime accelerator model matches the frozen calibration profile; provider versions and data geometry remain relevant."
  } else {
    "The compiled policy is being applied outside a confirmed hardware match. Method selection is unchanged; target and timing evidence are calibration-informed, not validated on this machine."
  }
  route
}
...
faiss_ivfpq_fastscan_normalized_metric_result <- function(data,
                                                          points,
                                                          k,
                                                          self_query,
                                                          exclude_self,
  ...
  metric <- normalize_nn_metric(metric)
  if (!metric %in% c("cosine", "correlation")) {
    stop("CPU IVFPQ FastScan normalized route currently supports cosine and correlation only.", call. = FALSE)
  }
  metric_inputs <- normalized_euclidean_metric_inputs(
    data,
    points,
  ...
  if (!is.null(cached)) {
    result <- finalize_normalized_euclidean_metric_result(cached, metric_inputs)
    approximation <- attr(result, "approximation", exact = TRUE)
    if (is.null(approximation)) approximation <- list()
    approximation$strategy <- "faiss_IndexIVFPQFastScan_RefineFlat_normalized_L2"
    approximation$metric <- metric
    ...
    approximation$distance_transform <- "normalized_euclidean_squared_over_2_to_1_minus_similarity"
    approximation$transform_storage <- metric_inputs$transform_storage %||% "double"
  ...
  result <- finalize_normalized_euclidean_metric_result(result, metric_inputs)
  attr(result, "approximation") <- list(
    strategy = "faiss_IndexIVFPQFastScan_RefineFlat_normalized_L2",
    backend = "faiss_ivfpq_fastscan",
    library = "faiss",
    accelerator = NULL,
    metric = metric,
    ...
    distance_transform = "normalized_euclidean_squared_over_2_to_1_minus_similarity",
...
nn_tuning_metadata <- function(params, prefix = NULL) {
  ...
  fields <- c(
    ...
    "expected_recall_at_k", "exact_recall_by_construction",
    "recommended_n_threads", "faiss_query_batch_size",
    "cache_fitted_indexes", "recommended_output", "result_backend",
    "resolved_backend", "distance_type", "input_type", "input_layout"
  )
...
native_nsg_params <- function(n, p, k, metric = "euclidean",
                              backend = c("cpu", "cuda"),
                              target_recall = 0.99) {
  metric <- normalize_nn_metric(metric)
...
cuda_nsg_params <- function(n, p, k, metric = "euclidean", target_recall = 0.99) {
  native_nsg_params(
    n = n, p = p, k = k, metric = metric,
    backend = "cuda", target_recall = target_recall
  )
}
...
vamana_params <- function(n, p, k, metric = "euclidean",
  ...
  backend <- match.arg(backend)
  target_recall <- normalize_hnsw_target_recall(target_recall)
  option_prefix <- if (identical(backend, "cuda")) "cuda_vamana" else "cpu_vamana"
  out <- nn_tune_vamana_cpp(
    ...
    nn_option_int_or_na(c(paste0(option_prefix, "_search_l"), "faiss_vamana_search_l")),
    {
      value <- faissr_option("vamana_alpha", NULL)
      backend_value <- faissr_option(paste0(option_prefix, "_alpha"), NULL)
...
nndescent_pool_size <- function(n, k, p = NA_integer_,
                                metric = "euclidean",
                                target_recall = 0.99) {
  as.integer(cpu_nndescent_params(n, k, p = p, metric = metric,
                                  target_recall = target_recall)$pool_size)
}

nndescent_iterations <- function(n, k, p = NA_integer_,
                                 metric = "euclidean",
                                 target_recall = 0.99) {
  as.integer(cpu_nndescent_params(n, k, p = p, metric = metric,
                                  target_recall = target_recall)$n_iters)
}

cpu_nndescent_params <- function(n, k, p = NA_integer_,
                                 metric = "euclidean",
                                 target_recall = 0.99) {
  n <- as.integer(n)
  k <- as.integer(k)
...
nndescent_self_knn <- function(data,
  ...
  params <- list(
    ...
    requested_target_recall = as.numeric(tune$requested_target_recall %||% target_recall),
    ...
    tuning_target_recall_code = as.integer(tune$tuning_target_recall_code %||% NA_integer_),
    ...
    tuning_benchmark_source = tune$tuning_benchmark_source %||% NA_character_,
...
ivfpq_fastscan_cpu_params <- function(n, p, k, target_recall = 0.99,
                                      metric = "euclidean") {
  metric <- normalize_nn_metric(metric)
  if (!metric %in% c("euclidean", "cosine", "correlation", "inner_product")) {
    stop("CPU IVFPQ FastScan tuning currently supports Euclidean, cosine, correlation, and inner-product metrics.", call. = FALSE)
  }
  ivf <- faiss_ivf_params(
    n, k, metric = metric, p = p,
    method = "ivfpq_fastscan", target_recall = target_recall
  )
  ...
  default_refine_factor <- ivfpq_fastscan_default_int(ivf$ivfpq_fastscan_refine_factor, 8L)
...
ivfpq_fastscan_cuda_params <- function(n, p, k, target_recall = 0.99,
  ...
  if (!manual_pq && length(tuned_pq_bits) == 1L && !is.na(tuned_pq_bits) &&
      is.finite(tuned_pq_bits) && tuned_pq_bits > 0L) {
  ...
  pq$tuning_target_recall_code <- ivf$tuning_target_recall_code %||% NA_integer_
...
faiss_ivfpq_pq_params <- function(p, n = NULL, ivf_params = NULL) {
  ...
  pq$requested_target_recall <- ivf_params$requested_target_recall %||% NA_real_
  ...
  pq$tuning_target_recall_code <- ivf_params$tuning_target_recall_code %||% NA_integer_
  pq$tuning_benchmark_basis <- ivf_params$tuning_benchmark_basis %||% NA_character_
  pq$tuning_benchmark_target_met <- ivf_params$tuning_benchmark_target_met %||% FALSE
  pq$tuning_benchmark_source <- ivf_params$tuning_benchmark_source %||% NA_character_
...
cuvs_cagra_params <- function(n, k, p = NA_integer_, metric = "euclidean", target_recall = 0.99) {
  target_recall <- normalize_hnsw_target_recall(target_recall)
  metric <- normalize_nn_metric(metric)
...
#' clustering. https://github.com/NVIDIA/cuvs.
#'
...
#'   correlation. CPU FastScan accepts the same metrics; cosine is implemented by row
...
#'   CUDA FastScan cosine and correlation auto policies are seeded from the CUDA Euclidean
...
nn <- function(data,
  ...
  result <- attach_nn_method_implementation_contract(
    result,
    requested_method = method,
    backend_used = result$backend_used %||% attr(result, "backend_used") %||%
...
gpu_knn_to_host <- function(x) {
  ...
  attach_nn_distance_contract(out, x$metric %||% attr(x, "metric") %||% "euclidean")
}
...
print.faissR_gpu_knn <- function(x, ...) {
  ...
  if (identical(x$metric %||% attr(x, "metric"), "inner_product")) {
    cat("  values:  query-specific shifted inner product; not a metric or ",
...
print.faissR_nn <- function(x, ...) {
  ...
  if (!is.null(x$implementation_label) &&
      identical(x$implementation_scope, "package_owned_style_implementation")) {
