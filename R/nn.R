...
attach_cpu_exact_tuning <- function(
    ...
    strategy <- switch(
        method,
        flat = paste0(backend_label, "_flat"),
        bruteforce = paste0(backend_label, "_bruteforce"),
        paste0(backend_label, "_exact")
    )
    attr_name <- switch(
        method,
        flat = "flat_tuning",
        bruteforce = "bruteforce_tuning",
        "exact_tuning"
    )
    meta <- c(
        list(
            ...
            resolved_backend = params$resolved_backend %||% "faiss_flat_l2",
            library = "faiss",
            method = method,
            exact = TRUE,
            exact_recall_by_construction = TRUE,
            expected_recall_at_k = as.numeric(
                params$expected_recall_at_k %||% 1
            ),
            metric = params$tuning_metric %||% NA_character_,
            actual_n_threads = as.integer(n_threads),
            recommended_n_threads = as.integer(
                params$recommended_n_threads %||% NA_integer_
            ),
            actual_output = output,
            recommended_output = params$recommended_output %||% NA_character_,
            faiss_query_batch_size = as.integer(
                params$faiss_query_batch_size %||% NA_integer_
            ),
            cache_fitted_indexes = isTRUE(params$cache_fitted_indexes)
        ),
        nn_tuning_metadata(params),
        extra %||% list()
    )
    ...
    attr(result, "faiss") <- c(
        provider,
        meta[setdiff(names(meta), names(provider))]
    )
    result
}
...
attach_cuda_exact_tuning <- function(
    ...
    library_label <- if (identical(method, "bruteforce")) "cuvs" else "faiss"
    strategy <- switch(
        method,
        flat = paste0(backend_label, "_flat"),
        bruteforce = paste0(backend_label, "_bruteforce"),
    ...
    meta <- c(
        list(
            ...
            actual_output = output,
            recommended_output = params$recommended_output %||% NA_character_,
            faiss_gpu_query_batch_size = as.integer(
                params$faiss_gpu_query_batch_size %||% NA_integer_
            ),
            faiss_gpu_reuse_resources = isTRUE(
                params$faiss_gpu_reuse_resources
            ),
            cache_fitted_indexes = FALSE
        ),
        nn_tuning_metadata(params),
        extra %||% list()
    )
    attr(result, attr_name) <- meta
    faiss <- attr(result, "faiss", exact = TRUE)
    if (is.null(faiss)) {
        faiss <- list()
    }
    attr(result, "faiss") <- c(faiss, meta[setdiff(names(meta), names(faiss))])
    result
}
...
fitted_nn_index_result <- function(
    ...
    cache_enabled <- if (is.null(use_cache)) {
        fitted_nn_index_cache_enabled()
    } else {
    ...
    out <- tryCatch(
        with_faiss_query_batch_size(params, {
            if (identical(kind, "hnsw")) {
    ...
    if (identical(kind, "flat")) {
        ...
        result <- attach_cpu_exact_tuning(
            result,
            params,
            output,
            n_threads,
    ...
    if (identical(kind, "ivfpq_fastscan")) {
        attr(result, "approximation") <- c(
            list(
                ...
                refine_parameters_adjusted = !identical(
                    as.integer(
                        params$requested_refine_factor %||%
                            out$requested_refine_factor
                    ),
                    as.integer(out$refine_factor)
                ) ||
                    !identical(
        ...
        result <- append_nn_tuning_metadata(
            ...
            pq,
            params,
            .prefixes = list("pq_", "ivfpq_fastscan_")
        )
        return(finish_float32_direct_result(result, out))
    }
...
nn_compute <- function(
    ...
    if (isTRUE(data_float32) || isTRUE(points_float32)) {
        ...
        if (isTRUE(exclude_self) && !isTRUE(self_query)) {
            stop(
                "Self-neighbor exclusion is only valid when ",
                "`points` is `data`.",
                call. = FALSE
            )
        }
        if (is.null(k)) {
            k <- if (data_dim[[1L]] == 1L) {
                ...
                min(
                    ...
                    auto_k(
                        data_dim[[1L]],
                        include_self = isTRUE(self_query) &&
                            !isTRUE(exclude_self)
                    )
                )
            }
        ...
        if (backend %in% c("cuda_auto", "gpu_auto")) {
            route <- auto_selection %||%
                nn_auto_selection_for_backend(
                    backend = "cuda_auto",
                    self_query = self_query,
                    n = data_dim[[1L]],
                    p = data_dim[[2L]],
                    n_points = points_dim[[1L]],
                    k = k,
                    work_size = as.double(data_dim[[1L]]) *
                        as.double(points_dim[[1L]]) *
                        as.double(data_dim[[2L]]),
                    metric = metric,
        ...
        if (metric %in% c("cosine", "correlation")) {
            if (backend %in% c("faiss_flat_cosine", "faiss_flat_correlation")) {
                if (!isTRUE(faiss_available())) {
                    stop(
                        "float32 normalized FAISS Flat input requires ",
                        "faissR to be built with FAISS.",
                        call. = FALSE
                    )
                }
            ...
            if (
                ...
                if (!isTRUE(faiss_gpu_available())) {
                    stop(
                        "float32 normalized FAISS GPU Flat input ",
                        "requires faissR to be built with FAISS GPU.",
                        call. = FALSE
                    )
                }
                return(faiss_flat_normalized_metric_result(
                    ...
                    exclude_self = isTRUE(exclude_self),
                    metric = metric,
                    backend = if (identical(metric, "correlation")) {
                        "faiss_gpu_flat_correlation"
                    } else {
                        "faiss_gpu_flat_cosine"
                    },
            ...
            if (
                backend %in%
                    c("faiss_ivf", "cpu_faiss_index_ivf", "faiss_ivf_flat")
            ) {
                if (!isTRUE(faiss_available())) {
                    stop(
                        "float32 normalized FAISS IVF input requires ",
                        "faissR to be built with FAISS.",
                        call. = FALSE
                    )
                }
            ...
            if (identical(backend, "faiss_ivfpq")) {
                if (!isTRUE(faiss_available())) {
                    stop(
                        "float32 normalized FAISS IVF-PQ input ",
                        "requires faissR to be built with FAISS.",
                        call. = FALSE
                    )
                }
            ...
            if (identical(backend, "faiss_hnsw")) {
                if (!isTRUE(faiss_available())) {
                    stop(
                        "float32 normalized FAISS HNSW input requires ",
                        "faissR to be built with FAISS.",
                        call. = FALSE
                    )
                }
            ...
            if (
                ...
                if (!isTRUE(faiss_gpu_available())) {
                    stop(
                        "float32 normalized FAISS GPU IVF input ",
                        "requires faissR to be built with FAISS GPU.",
                        call. = FALSE
                    )
                }
                params <- cuda_ivf_params(
                    data_dim[[1L]],
                    data_dim[[2L]],
                    k,
                    metric = metric,
            ...
            if (backend %in% c("faiss_gpu_ivfpq", "cuda_faiss_ivfpq")) {
                if (!isTRUE(faiss_gpu_available())) {
                    stop(
                        "float32 normalized FAISS GPU IVF-PQ input ",
                        "requires faissR to be built with FAISS GPU.",
                        call. = FALSE
                    )
                }
        ...
        if (
            backend %in% c("faiss_ivf", "cpu_faiss_index_ivf", "faiss_ivf_flat")
        ) {
            if (!metric %in% c("euclidean", "inner_product")) {
                stop(
                    "float32 FAISS IVF input currently supports ",
                    "`metric = \"euclidean\"` or ",
                    "`\"inner_product\"`.",
                    call. = FALSE
                )
            ...
            if (!isTRUE(faiss_available())) {
                stop(
                    "float32 FAISS IVF input requires faissR to be ",
                    "built with FAISS.",
                    call. = FALSE
                )
            }
        ...
        if (identical(backend, "faiss_nsg")) {
            if (!identical(metric, "euclidean")) {
                stop(
                    "float32 FAISS NSG input currently supports ",
                    "`metric = \"euclidean\"`.",
                    call. = FALSE
                )
            }
            if (!isTRUE(faiss_available())) {
                stop(
                    "float32 FAISS NSG input requires faissR to be ",
                    "built with FAISS.",
                    call. = FALSE
                )
            }
        ...
        if (identical(backend, "faiss_nndescent")) {
            if (!identical(metric, "euclidean")) {
                stop(
                    "float32 FAISS NNDescent input currently ",
            ...
            if (!isTRUE(faiss_available())) {
                stop(
                    "float32 FAISS NNDescent input requires faissR ",
                    "to be built with FAISS.",
                    call. = FALSE
                )
            }
        ...
        if (
            backend %in%
                c("cuvs_bruteforce", "cuda_cuvs_bruteforce", "cuda_cuvs_exact")
        ) {
            if (
                !metric %in%
                ...
                stop(
                    "float32 cuVS brute-force input currently ",
                    "supports `metric = \"euclidean\"`, ",
            ...
            brute_params <- cuda_bruteforce_params(
                data_dim[[1L]],
                data_dim[[2L]],
                k,
                metric = metric,
        ...
        if (backend %in% c("cuda_cuvs_cagra", "cuda_cagra", "gpu_cagra")) {
            ...
            use_float32_transform <- identical(
                metric_inputs$transform_storage %||% "double",
                "float32"
            )
            use_float32_input <- is.null(metric_inputs) ||
                isTRUE(use_float32_transform)
            distance_output <- if (is.null(metric_inputs)) output else "double"
            params <- cuvs_cagra_params(
                data_dim[[1L]],
                k,
                p = data_dim[[2L]],
                metric = metric,
            ...
            build_algo <- cuvs_cagra_build_algo_for(
                search_data,
                k,
                self_query,
                params
            ...
            attr(result, "approximation") <- list(
                ...
                input_type = out$input_type %||%
                    if (isTRUE(use_float32_input)) "float32" else "double",
                graph_degree = as.integer(out$graph_degree),
                intermediate_graph_degree = as.integer(
                    out$intermediate_graph_degree
                ),
        ...
        if (identical(backend, "faiss_ivfpq_fastscan")) {
            if (
                ...
                stop(
                    "float32 FAISS IVFPQ FastScan input currently ",
                    "supports `metric = \"euclidean\"`, ",
                    "`\"cosine\"`, `\"correlation\"`, or ",
                    "`\"inner_product\"`.",
                    call. = FALSE
                )
            }
            ...
            validate_faiss_cpu_ivfpq_training_size(data_dim[[1L]])
            params <- ivfpq_fastscan_cpu_params(
                data_dim[[1L]],
                data_dim[[2L]],
                k,
                target_recall = target_recall,
            ...
            if (metric %in% c("cosine", "correlation")) {
                return(faiss_ivfpq_fastscan_normalized_metric_result(
                    data = data,
                    points = points,
                    k = k,
                    self_query = self_query,
        ...
        if (identical(backend, "faiss_ivfpq")) {
            if (!metric %in% c("euclidean", "inner_product")) {
                stop(
                    "float32 FAISS IVF-PQ input currently supports ",
                    "`metric = \"euclidean\"` or ",
                    "`\"inner_product\"`.",
                    call. = FALSE
                )
            ...
            if (!isTRUE(faiss_available())) {
                stop(
                    "float32 FAISS IVF-PQ input requires faissR to ",
                    "be built with FAISS.",
                    call. = FALSE
                )
            }
        ...
        if (
            ...
            if (!metric %in% c("euclidean", "inner_product")) {
                stop(
                    "float32 FAISS GPU Flat input currently ",
                    "supports `metric = \"euclidean\"` or ",
                    "`\"inner_product\"`.",
                    call. = FALSE
                )
            }
            if (!isTRUE(faiss_gpu_available())) {
                stop(
                    "float32 FAISS GPU Flat input requires faissR ",
                    "to be built with FAISS GPU.",
                    call. = FALSE
                )
            }
            flatlike_params <- if (
                metric %in% c("euclidean", "inner_product")
            ) {
                cuda_flatlike_params(
                    data_dim[[1L]],
                    data_dim[[2L]],
                    k,
                    metric = metric,
        ...
        if (
            backend %in%
                c("faiss_gpu_ivf", "faiss_gpu_ivf_flat", "cuda_faiss_ivf_flat")
        ) {
            if (!metric %in% c("euclidean", "inner_product")) {
                stop(
                    "float32 FAISS GPU IVF-Flat input currently ",
                    "supports `metric = \"euclidean\"` or ",
                    "`\"inner_product\"`.",
                    call. = FALSE
                )
            ...
            if (!isTRUE(faiss_gpu_available())) {
                stop(
                    "float32 FAISS GPU IVF-Flat input requires ",
                    "faissR to be built with FAISS GPU.",
                    call. = FALSE
                )
            }
            params <- cuda_ivf_params(
        ...
        if (backend %in% c("faiss_gpu_ivfpq", "cuda_faiss_ivfpq")) {
            if (!metric %in% c("euclidean", "inner_product")) {
                stop(
                    "float32 FAISS GPU IVF-PQ input currently ",
                    "supports `metric = \"euclidean\"` or ",
                    "`\"inner_product\"`.",
                    call. = FALSE
                )
            ...
            if (!isTRUE(faiss_gpu_available())) {
                stop(
                    "float32 FAISS GPU IVF-PQ input requires ",
                    "faissR to be built with FAISS GPU.",
                    call. = FALSE
                )
            }
        ...
        if (backend %in% c("faiss_gpu_cagra", "cuda_faiss_cagra")) {
            if (!isTRUE(faiss_gpu_available())) {
                stop(
                    "float32 FAISS GPU CAGRA input requires faissR ",
                    "to be built with FAISS GPU.",
                    call. = FALSE
                )
            }
        ...
        if (backend %in% grid_backends) {
            if (!isTRUE(self_query)) {
                stop(
                    "Grid nearest-neighbour search is currently ",
                    "available for self-KNN searches only.",
                    call. = FALSE
                )
            }
            if (identical(metric, "inner_product")) {
                stop(
                    "Grid nearest-neighbour search does not ",
                    "support `metric = \"inner_product\"`.",
                    call. = FALSE
                )
            }
            if (!data_dim[[2L]] %in% c(2L, 3L)) {
                stop(
                    "Grid nearest-neighbour search supports only ",
                    "two- or three-column matrices.",
                    call. = FALSE
                )
            }
            use_cuda <- backend %in%
            ...
            if (isTRUE(use_cuda) && !isTRUE(cuda_available())) {
                stop(
                    "No CUDA GPU backend is available on this machine.",
                    call. = FALSE
                )
            }
            metric_inputs <- NULL
        ...
        direct_float32_backend <- backend %in%
            c(
                "faiss",
                "cpu_faiss",
                "cpu_faiss_flat",
        ...
        if (!isTRUE(direct_float32_backend)) {
            stop(
                "float32 input for backend `",
                backend,
                "` has no direct float32 ",
                "adapter yet. faissR no longer converts ",
                "float32 inputs back to R ",
        ...
        if (backend %in% c("cuvs_ivf_flat", "cuda_cuvs_ivf_flat")) {
            if (!identical(metric, "euclidean")) {
                stop(
                    "float32 cuVS IVF-Flat input currently ",
                    "supports `metric = \"euclidean\"`.",
                    call. = FALSE
                )
            }
        ...
        if (backend %in% c("cuda_cuvs_ivfpq_fastscan", "cuvs_ivfpq_fastscan")) {
            if (
                ...
                stop(
                    "float32 CUDA cuVS IVFPQ FastScan input ",
            ...
            use_float32_output <- identical(output, "float") &&
                is.null(metric_inputs)
            params_p <- if (is.null(metric_inputs)) {
                data_dim[[2L]]
            } else {
                ncol(search_data)
            }
            ...
            attr(result, "approximation") <- c(
                list(
                    ...
                    note = if (identical(metric, "cosine")) {
                        paste0(
                            "CUDA cosine FastScan route row-normalizes to ",
                            "float32, then uses cuVS IVF-PQ with 4-bit ",
                            "compressed codes."
                        )
                    } else if (identical(metric, "correlation")) {
                        paste0(
                            "CUDA correlation FastScan route row-centers ",
                            "and row-normalizes to float32, then uses cuVS ",
                            "IVF-PQ with 4-bit compressed codes."
                        )
                    } else if (identical(metric, "inner_product")) {
                        paste0(
                            "CUDA raw-inner-product FastScan route applies ",
                            "the maximum-inner-product-to-L2 ",
                            "extra-dimension transform, then uses cuVS ",
                            "IVF-PQ with 4-bit compressed codes."
                        )
                    } else {
                        paste0(
                            "CUDA route uses cuVS IVF-PQ with 4-bit ",
                            "compressed codes; FAISS FastScan is CPU-only ",
                            "in this package route."
                        )
                    },
                    nlist = as.integer(out$n_lists),
                    nprobe = as.integer(out$n_probes),
                    ...
                    pq_parameters_adjusted = isTRUE(
                        out$pq_parameters_adjusted
                    ) ||
                        !identical(
                            as.integer(params$pq$requested_pq_dim),
                            as.integer(out$pq_dim)
                        ) ||
                        !identical(
                            as.integer(params$pq$requested_pq_bits),
        ...
        if (
            ...
            if (!identical(metric, "euclidean")) {
                stop(
                    "float32 cuVS IVF-PQ input currently supports ",
                    "`metric = \"euclidean\"`.",
                    call. = FALSE
                )
            }
            ...
            attr(result, "approximation") <- list(
                ...
                pq_parameters_adjusted = isTRUE(out$pq_parameters_adjusted) ||
                    !identical(
                        as.integer(pq$requested_pq_dim),
                        as.integer(out$pq_dim)
                    ) ||
                    !identical(
                        as.integer(pq$requested_pq_bits),
                        as.integer(out$pq_bits)
                    ),
                pq_alignment_adjusted = isTRUE(out$pq_alignment_adjusted) ||
                    isTRUE(pq$pq_alignment_adjusted),
                pq_alignment_rule = out$pq_alignment_rule %||%
                    pq$pq_alignment_rule %||%
                    NA_character_,
                search_batch_size = as.integer(out$search_batch_size)
            )
            result <- append_nn_tuning_metadata(
        ...
        if (
            backend %in%
                c("cuvs_nndescent", "cuda_cuvs_nndescent", "cuda_nndescent")
        ) {
            if (identical(metric, "inner_product")) {
                stop(
                    "cuVS NN-descent does not support raw ",
                    ...
                    "distinct reference and query transforms.",
                    call. = FALSE
                )
            }
            ...
            if (!isTRUE(self_query)) {
                stop(
                    "`backend = \"cuda_cuvs_nndescent\"` is only ",
                    "available for self-KNN searches.",
                    call. = FALSE
                )
            }
            reject_cuda_r_side_output_cleanup(
                "cuda_cuvs_nndescent",
                exclude_self
            )
            metric_inputs <- NULL
            ...
            attr(result, "approximation") <- list(
                ...
                intermediate_graph_degree = as.integer(
                    out$intermediate_graph_degree %||% NA_integer_
                ),
                max_iterations = as.integer(
                    out$max_iterations %||% NA_integer_
                ),
                cuvs_nndescent_input_dtype = out$cuvs_nndescent_input_dtype %||%
                    "float32",
        ...
        if (identical(backend, "faiss_hnsw")) {
            if (!metric %in% c("euclidean", "inner_product")) {
                stop(
                    "float32 FAISS HNSW input currently supports ",
                    "`metric = \"euclidean\"` ",
                    "or `\"inner_product\"`.",
                    call. = FALSE
                )
            ...
            if (!isTRUE(faiss_available())) {
                stop(
                    "float32 FAISS HNSW input requires faissR to ",
                    "be built with FAISS.",
                    call. = FALSE
                )
            }
        ...
        if (identical(backend, "cpu_nndescent")) {
            if (!isTRUE(self_query)) {
                stop(
                    "`method = \"nndescent\"` is only available ",
                    "for self-KNN searches on CPU.",
                    call. = FALSE
                )
            }
            metric_inputs <- NULL
        ...
        if (backend %in% c("cpu_nsg", "cuda_nsg")) {
            if (!isTRUE(self_query)) {
                stop(
                    "Native NSG is currently implemented for ",
                    "self-KNN searches only.",
                    call. = FALSE
                )
            }
            ...
            attr(result, "approximation") <- c(
                list(
                    ...
                    requested_seed_backend = params$seed_backend %||%
                        NA_character_,
                    seed_k = as.integer(params$seed_k %||% params$graph_k),
                    graph_k_cap = as.integer(params$graph_k_cap),
                    nsg_parameters_adjusted = !identical(
                        as.integer(params$r),
                        as.integer(params$requested_r)
                    ) ||
                        !identical(
                            as.integer(params$graph_k),
                    ...
                    tuning_shape_group = params$tuning_shape_group %||%
                        NA_character_,
                    tuning_k_bucket = as.integer(
                        params$tuning_k_bucket %||% NA_integer_
                    ),
                    tuning_target_recall_code = as.integer(
        ...
        if (backend %in% c("cpu_vamana", "cuda_vamana")) {
            if (!isTRUE(self_query)) {
                stop(
                    "Vamana is currently implemented for self-KNN ",
                    "searches only.",
                    call. = FALSE
                )
            }
        ...
        if (
            ...
            stop(
                "float32 FAISS Flat input currently supports ",
                "`metric = \"euclidean\"`, ",
        ...
        if (!isTRUE(faiss_available())) {
            stop(
                "float32 FAISS Flat input requires faissR to ",
                "be built with FAISS.",
                call. = FALSE
            )
        }
        ...
        exact_params <- cpu_flatlike_params(
            data_dim[[1L]],
            data_dim[[2L]],
            k,
            metric = metric,
    ...
    if (is.null(k)) {
        k <- if (nrow(data) == 1L) {
            ...
            min(
                ...
                auto_k(
                    nrow(data),
                    include_self = isTRUE(self_query) && !isTRUE(exclude_self)
                )
            )
        }
    }
    ...
    if (!identical(metric, "euclidean")) {
        if (
            ...
            backend %in% c("faiss_flat_cosine", "faiss_flat_correlation")
        ) {
            backend <- backend
        } else if (
            identical(metric, "cosine") &&
            ...
            backend %in%
                c("faiss_gpu_flat_cosine", "faiss_gpu_flat_correlation")
        ) {
            backend <- backend
            ...
            stop(
                "`metric = \"",
                metric,
                "\"` currently supports only `backend = \"cpu\"` ",
                "or a validated metric-specific exact backend. ",
                "Approximate FAISS, CUDA, and cuVS KNN paths in this build ",
                "have validated Euclidean-distance semantics ",
                "only unless explicitly routed.",
                call. = FALSE
            )
        }
    }
    ...
    work_size <- as.double(nrow(data)) *
        as.double(nrow(points)) *
        as.double(ncol(data))
    ...
    if (backend %in% c("auto", "cpu_auto", "cuda_auto", "gpu_auto")) {
        ...
        if (!isTRUE(self_query)) {
            stop(
                "`backend = \"cpu_approx\"` is only available ",
                "for self-KNN searches.",
                call. = FALSE
            )
        }
    ...
    if (
        ...
        if (metric %in% c("euclidean", "inner_product")) {
            cached <- fitted_nn_index_result(
                ...
                points = points,
                k = k,
                backend = if (identical(metric, "inner_product")) {
                    "faiss_flat_ip"
                } else {
                    "faiss_flat_l2"
                },
    ...
    if (
        backend %in%
            c("faiss_gpu_flat", "faiss_gpu_flat_l2", "cuda_faiss_flat_l2")
    ) {
        if (!isTRUE(faiss_gpu_available())) {
            stop(
                "The real FAISS C++ GPU Flat L2 backend is not ",
        ...
        flatlike_params <- cuda_flatlike_params(
            nrow(data),
            ncol(data),
            k,
            metric = "euclidean",
    ...
    if (backend %in% c("faiss_gpu_flat_ip", "cuda_faiss_flat_ip")) {
        if (!isTRUE(faiss_gpu_available())) {
            stop(
                "The real FAISS C++ GPU Flat IP backend is not ",
                "available in this build. ",
                "Reinstall faissR with FAISS GPU/cuVS headers ",
                "available through `FAISS_HOME`.",
                call. = FALSE
    ...
    if (
        ...
        metric_label <- if (
            backend %in%
                c("faiss_gpu_flat_correlation", "cuda_faiss_flat_correlation")
        ) {
            "correlation"
        } else {
            "cosine"
        ...
        return(faiss_flat_normalized_metric_result(
            ...
            exclude_self = isTRUE(exclude_self),
            metric = metric_label,
            backend = if (identical(metric_label, "correlation")) {
                "faiss_gpu_flat_correlation"
            } else {
                "faiss_gpu_flat_cosine"
            },
    ...
    if (backend %in% c("faiss_ivf", "cpu_faiss_index_ivf", "faiss_ivf_flat")) {
        if (!isTRUE(faiss_available())) {
            stop(
                "The real FAISS C++ IVF backend is not ",
                "available in this build. ",
                "Reinstall faissR with `FAISS_HOME` pointing ",
                "to a FAISS installation.",
                call. = FALSE
    ...
    if (identical(backend, "faiss_ivfpq")) {
        if (!isTRUE(faiss_available())) {
            stop(
                "The real FAISS C++ IVFPQ backend is not ",
                "available in this build. ",
                "Reinstall faissR with `FAISS_HOME` pointing ",
                "to a FAISS installation.",
                call. = FALSE
    ...
    if (identical(backend, "faiss_ivfpq_fastscan")) {
        if (
            ...
            stop(
                "FAISS IVFPQ FastScan input currently supports ",
        ...
        if (!isTRUE(faiss_fastscan_available())) {
            stop(
                "The FAISS IVFPQ FastScan backend is not ",
                "available in this build. ",
                "Reinstall faissR with `FAISS_HOME` pointing ",
                "to a FAISS installation ",
    ...
    if (
        ...
        if (!isTRUE(faiss_gpu_available())) {
            stop(
                "The real FAISS C++ GPU IVF Flat backend is ",
        ...
        if (
            isTRUE(faiss_gpu_ivf_should_tune(
                data,
                k,
                self_query,
                tuning = tuning,
    ...
    if (backend %in% c("faiss_gpu_ivfpq", "cuda_faiss_ivfpq")) {
        if (!isTRUE(faiss_gpu_available())) {
            stop(
                "The real FAISS C++ GPU IVF-PQ backend is not ",
                "available in this build. ",
                "Reinstall faissR with FAISS GPU/cuVS headers ",
                "available through `FAISS_HOME`.",
                call. = FALSE
    ...
    if (backend %in% c("faiss_gpu_cagra", "cuda_faiss_cagra")) {
        if (!isTRUE(faiss_gpu_available())) {
            stop(
                "The real FAISS GPU CAGRA backend is not ",
                "available in this build. ",
                "Reinstall faissR with FAISS GPU/cuVS headers ",
                "available through `FAISS_HOME`.",
                call. = FALSE
        ...
        requested_graph_degree <- if (is.null(params$requested_graph_degree)) {
            out$requested_graph_degree
        } else {
            params$requested_graph_degree
        }
        requested_intermediate_graph_degree <- if (
            is.null(params$requested_intermediate_graph_degree)
        ) {
            out$requested_intermediate_graph_degree
        } else {
            params$requested_intermediate_graph_degree
        }
        requested_search_width <- if (is.null(params$requested_search_width)) {
            out$requested_search_width
        } else {
            params$requested_search_width
        }
        requested_itopk_size <- if (is.null(params$requested_itopk_size)) {
            out$requested_itopk_size
        } else {
            params$requested_itopk_size
        }
        result <- finish_nn_result(
            out,
            "faiss_gpu_cagra",
        ...
        attr(result, "approximation") <- list(
            ...
            cagra_parameters_adjusted = isTRUE(out$cagra_parameters_adjusted) ||
                !identical(
                    as.integer(requested_graph_degree),
                    as.integer(out$graph_degree)
                ) ||
                !identical(
                    as.integer(requested_intermediate_graph_degree),
                    as.integer(out$intermediate_graph_degree)
                ) ||
                !identical(
                    as.integer(requested_search_width),
                    as.integer(out$search_width)
                ) ||
                !identical(
                    as.integer(requested_itopk_size),
    ...
    if (identical(backend, "faiss_hnsw")) {
        if (!isTRUE(faiss_available())) {
            stop(
                "The real FAISS C++ HNSW backend is not ",
                "available in this build. ",
                "Reinstall faissR with `FAISS_HOME` pointing ",
                "to a FAISS installation.",
                call. = FALSE
    ...
    if (identical(backend, "faiss_nsg")) {
        if (!isTRUE(faiss_available())) {
            stop(
                "The real FAISS C++ NSG backend is not ",
                "available in this build. ",
                "Reinstall faissR with `FAISS_HOME` pointing ",
                "to a FAISS installation.",
                call. = FALSE
        ...
        if (metric %in% c("cosine", "correlation", "inner_product")) {
            stop(
                "`backend = \"faiss_nsg\"` currently supports ",
                ...
                "routes in this linked FAISS build.",
                call. = FALSE
            )
        }
        params <- faiss_nsg_params(k)
    ...
    if (backend %in% c("cpu_vamana", "cuda_vamana")) {
        if (!isTRUE(self_query)) {
            stop(
                "Vamana is currently implemented for self-KNN searches only.",
                call. = FALSE
            )
        }
        use_cuda <- identical(backend, "cuda_vamana")
    ...
    if (identical(backend, "faiss_nndescent")) {
        if (!identical(metric, "euclidean")) {
            stop(
                "`backend = \"faiss_nndescent\"` is currently ",
                "validated only for ",
        ...
        if (!isTRUE(faiss_available())) {
            stop(
                "The real FAISS C++ NNDescent backend is not ",
                "available in this build. ",
                "Reinstall faissR with `FAISS_HOME` pointing ",
                "to a FAISS installation.",
                call. = FALSE
    ...
    if (backend %in% c("cuda_cuvs_cagra", "cuda_cagra", "gpu_cagra")) {
        ...
        if (
            ...
            tuning_metadata <- tuned$tuning
            if (
                is.list(tuning_metadata) &&
                    !identical(tuning_metadata$status, "target_met")
            ) {
                best_recall <- if (
                    is.data.frame(tuning_metadata$results) &&
                        "recall" %in% names(tuning_metadata$results)
                ) {
                    faissr_quiet_warning(max(
                        tuning_metadata$results$recall,
                        na.rm = TRUE
                    ))
                } else {
                    NA_real_
                }
                min_recall <- faissr_option(
                    "cuvs_cagra_tune_min_recall",
                    tuning_metadata$target_recall
                )
                min_recall <- faissr_quiet_warning(as.numeric(min_recall))
                if (
                    length(min_recall) != 1L ||
                        is.na(min_recall) ||
                        !is.finite(min_recall)
                ) {
                    min_recall <- 0.985
                }
                if (!is.finite(best_recall) || best_recall < min_recall) {
                    stop(
                        "cuVS CAGRA pilot tuning did not meet the ",
                        "requested recall target ",
                        "on this dataset (best pilot recall = ",
                        if (is.finite(best_recall)) {
                            formatC(best_recall, digits = 4, format = "f")
                        } else {
                            "NA"
                        },
                        "). Use `backend = \"faiss_gpu_cagra\"`, ",
                        "`backend = \"cuda_cuvs_bruteforce\"`, or ",
                        "explicitly disable ",
        ...
        out <- if (
            isTRUE(use_float32_transform) || isTRUE(use_float32_output)
        ) {
            nn_cuvs_cagra_float32_cpp(
                search_data,
                search_points,
        ...
        attr(result, "approximation") <- list(
            ...
            search_width = as.integer(out$search_width),
            itopk_size = as.integer(out$itopk_size),
            cagra_build_algo = out$build_algo %||%
                cagra_build_algo_preference(),
            nn_descent_niter = as.integer(
                out$nn_descent_niter %||% NA_integer_
            ),
            requested_graph_degree = as.integer(requested_graph_degree),
            requested_intermediate_graph_degree = as.integer(
                requested_intermediate_graph_degree
    ...
    if (backend %in% c("cuda_cuvs_ivfpq_fastscan", "cuvs_ivfpq_fastscan")) {
        if (
            ...
            stop(
                "CUDA cuVS IVFPQ FastScan input currently ",
                "supports `metric = \"euclidean\"`, ",
                "`\"cosine\"`, `\"correlation\"`, or ",
                "`\"inner_product\"`.",
                call. = FALSE
    ...
    if (
        ...
        use_float32_output <- identical(output, "float") &&
            is.null(metric_inputs)
        brute_distance_output <- if (isTRUE(use_float32_output)) {
            output
        } else {
            "double"
        }
    ...
    if (
        ...
        if (!isTRUE(self_query)) {
            stop(
                "`backend = \"cuda_grid_auto\"` is only ",
                "available for self-KNN searches.",
                call. = FALSE
            )
        }
        if (identical(metric, "inner_product")) {
            stop(
                "`backend = \"cuda_grid_auto\"` does not ",
                "support inner-product search.",
                call. = FALSE
            )
        }
        if (!ncol(data) %in% c(2L, 3L)) {
            stop(
                "`backend = \"cuda_grid_auto\"` supports only ",
                "two- or three-column matrices.",
                call. = FALSE
            )
        }
    ...
    if (
        ...
        if (!isTRUE(self_query)) {
            stop(
                "`backend = \"cpu_grid\"` is only available ",
                "for self-KNN searches.",
                call. = FALSE
            )
        }
        if (identical(metric, "inner_product")) {
            stop(
                "`backend = \"cpu_grid\"` does not support ",
        ...
        if (!is.null(metric_inputs) && backend %in% c("grid", "cpu_grid")) {
            grid_backend <- if (ncol(search_data) == 3L) {
                "cpu_grid3d"
            } else {
                "cpu_grid2d"
            }
        ...
        result <- finish_nn_result(
            out,
            attr(out, "spatial_index")$backend,
            k,
            self_query,
            exact = TRUE,
            metric = metric
        ...
        attr(result, "spatial_index")$metric_transform <- if (
            is.null(metric_inputs)
        ) {
            NA_character_
        } else {
    ...
    if (!is.na(selected_gpu)) {
        if (selected_gpu == "cuda") {
            ...
            if (isTRUE(exclude_self)) {
                ...
                stop(
                    "`backend = \"cuda\"` with `exclude_self = ",
                    "TRUE` requires cuVS ",
                    "brute force or FAISS GPU Flat so ",
                    "self-neighbor removal is handled ",
            ...
            if (gpu_k > 256L) {
                stop(
                    "Native GPU backends currently support `k <= 256`.",
                    call. = FALSE
                )
            }
...
nn_resolved_backend_available <- function(backend) {
    ...
    if (startsWith(backend, "cuda_cuvs") || startsWith(backend, "cuvs")) {
        return(nn_dependency_availability(cuvs_available, "cuvs", "cuVS"))
    }
...
nn_capability_row <- function(method, backend, metric) {
    ...
    if (identical(backend, "auto")) {
        ...
        notes <- if (supported) {
            paste(
                paste0(
                    "Auto backend uses CUDA only when the selected ",
                    "method/metric has a"
                ),
                paste0(
                    "validated CUDA route and the required runtime ",
        ...
        if (identical(backend, "cpu")) {
            ...
            notes <- paste0(
                "Euclidean can resolve to exact, grid, FAISS ",
                "IVF, FAISS HNSW, native CPU NSG/Vamana ",
                "refinement, or native CPU NN-descent ",
                "depending on shape; non-Euclidean resolves to ",
                "exact, FAISS Flat, FAISS HNSW, native CPU ",
                "NSG/Vamana refinement, or native CPU ",
                "NN-descent depending on shape."
            )
        } else if (all_metrics) {
            supported <- TRUE
            exact <- NA
            ...
            notes <- if (euclidean) {
                paste0(
                    "Can resolve to CUDA grid, exact FAISS GPU ",
                    "Flat/cuVS brute force, or IVF-Flat by shape, ",
                    "k, target_recall, and availability."
                )
            } else if (metric %in% c("cosine", "correlation")) {
                paste0(
                    "CUDA auto can resolve to CUDA grid for large ",
                    "2D/3D self-search, FAISS GPU Flat ",
                    "normalized-IP search for exact small/query ",
                    "workloads, or FAISS GPU/direct cuVS CAGRA ",
                    "graph routes for large self-KNN when ",
                    "available; cuVS-only runtimes are ",
                    "shape-dependent and keep small/query ",
                    "workloads on CPU."
                )
            } else {
                paste0(
                    "CUDA auto uses FAISS GPU Flat IP for exact ",
                    "small/query inner-product search when FAISS ",
                    "GPU Flat is available, or transformed FAISS ",
                    "GPU/direct cuVS CAGRA for large self-KNN ",
                    "graph search when available."
                )
            }
        }
        ...
        if (identical(backend, "cpu")) {
            ...
            notes <- paste0(
                "CPU exact search supports all public metrics ",
                "through FAISS Flat routes, with cosine and ",
                "correlation using normalized Flat IP ",
                "internally."
            )
        } else {
            if (euclidean) {
                ...
                notes <- paste0(
                    "Euclidean CUDA exact/brute-force search can ",
                    "use FAISS GPU Flat when available, otherwise ",
                    "direct cuVS brute force when cuVS is ",
                    "available."
                )
            } else {
                implementation <- paste0(
                    "FAISS GPU Flat or transformed cuVS ",
                    "brute force"
                )
                notes <- paste0(
                    "CUDA cosine and correlation exact/brute-force ",
                    "search can use FAISS GPU Flat metric-aware ",
                    "routes or direct cuVS brute force with exact ",
                    "metric transforms."
                )
            }
        }
        ...
        notes <- paste0(
            "Cosine uses row L2 normalization plus Flat ",
            "IP; correlation uses row centering plus L2 ",
            "normalization plus Flat IP."
        )
        ...
        notes <- if (supported) {
            paste0(
                "Only valid for 2D/3D self-KNN. ",
                "Cosine/correlation use normalized Euclidean ",
                "grid search."
            )
        } else {
        ...
        notes <- if (identical(backend, "cpu")) {
            paste0(
                "Uses FAISS HNSW for all metrics; cosine and ",
                "correlation use normalized inner-product ",
                "HNSW."
            )
        } else {
            paste0(
                "Uses RAPIDS cuVS HNSW by building a CAGRA ",
                "seed graph and converting it with the host ",
                "dataset. Metadata records cuda_hnsw_design = ",
                "cuvs_hnsw_from_cagra_cpu_hierarchy because ",
                "this is not a pure all-GPU HNSW search ",
                "implementation."
            )
        }
        ...
        notes <- if (identical(method, "ivf")) {
            paste0(
                "FAISS IVF-Flat supports Euclidean/L2; ",
                "cosine/correlation use row transforms ",
                "followed by IVF search."
            )
        } else {
            paste0(
                "FAISS IVFPQ supports Euclidean/L2; ",
                "cosine/correlation use row transforms ",
                "followed by IVFPQ search."
            )
        }
        ...
        notes <- if (!supported) {
            if (identical(backend, "cpu")) {
                paste0(
                    "CPU IVFPQ FastScan supports Euclidean/L2, ",
                    "cosine, and correlation."
                )
            } else {
                paste0(
                    "CUDA IVFPQ FastScan supports Euclidean/L2, ",
                    "cosine, and correlation."
                )
            }
        } else if (identical(backend, "cpu")) {
            paste0(
                "Uses FAISS IndexIVFPQFastScan with 4-bit PQ ",
                "lookup tables and optional Flat reranking; ",
                "cosine normalizes rows and correlation ",
                "centers/normalizes rows before FastScan L2."
            )
        } else {
            paste0(
                "Uses direct cuVS IVF-PQ with 4-bit compressed ",
                "codes; cosine row-normalizes, correlation ",
                "row-centers plus row-normalizes. The route ",
                "does not silently fall back to CPU FAISS ",
                "FastScan."
            )
        }
        ...
        notes <- if (identical(backend, "cpu")) {
            paste0(
                "Public CPU NSG uses faissR's distinct ",
                "NSG/MRNG-derived candidate-graph algorithm ",
                "for all metrics to avoid unsafe linked-FAISS ",
                "graph construction; large high-dimensional ",
                "CPU inputs use a deterministic FAISS HNSW ",
                "seed before compiled C++ derived pruning over ",
                "compact candidate storage."
            )
        } else if (supported) {
            paste0(
                "CUDA NSG builds faissR's distinct ",
                "NSG/MRNG-derived candidate graph, prunes it ",
                "in compiled C++ over compact candidate ",
                "storage, and refines candidates with the ",
        ...
        notes <- if (identical(backend, "cpu")) {
            paste0(
                "Builds faissR's distinct ",
                "DiskANN/Vamana-derived robust-pruned ",
                "candidate graph using compiled C++ pruning ",
                "over compact candidate storage and refines ",
                "top-k within candidate rows on CPU; large ",
                "high-dimensional CPU inputs use a ",
                "deterministic FAISS HNSW seed before robust ",
                "pruning. Cosine/correlation use normalized ",
                "Euclidean search."
            )
        } else {
            paste0(
                "Builds faissR's distinct Vamana-derived ",
                "candidate graph using compiled C++ pruning ",
                "over compact candidate storage and refines ",
                "candidate rows with the native CUDA ",
                "row-candidate kernel; cuVS Vamana currently ",
                "builds/serializes DiskANN-compatible indexes ",
                "but does not expose KNN search."
            )
        }
        ...
        notes <- if (identical(backend, "cpu")) {
            paste0(
                "Native CPU NNDescent supports Euclidean/L2 ",
                "self-KNN; cosine/correlation use normalized ",
                "Euclidean graph search. Seed neighbours use ",
                "random-projection windows plus deterministic ",
                "row fill, with flat row-major graph buffers ",
                "and fixed-width reverse-neighbour storage in ",
                "C++."
            )
        } else if (supported) {
            paste0(
                "CUDA NN-descent uses direct RAPIDS cuVS ",
                "NN-descent for Euclidean/L2 self-KNN; ",
                "cosine/correlation use normalized Euclidean ",
                "graph search."
            )
        }
        ...
        exact <- if (supported) FALSE else NA
        implementation <- if (identical(backend, "cuda")) {
        ...
        notes <- if (!identical(backend, "cuda")) {
            ...
            paste0(
                "CUDA-only approximate graph search; ",
                "faissR.cagra_implementation selects FAISS GPU ",
                "CAGRA, direct cuVS CAGRA, or a deterministic ",
                "shape-aware auto provider rule."
            )
        } else {
...
resolve_public_nn_backend <- function(
    ...
    if (identical(device, "cpu")) {
        if (identical(method, "grid") && identical(metric, "inner_product")) {
            stop(
                "CPU `method = \"grid\"` does not support ",
                "`metric = \"inner_product\"`.",
                call. = FALSE
            )
        }
        if (
            ...
            stop(
                "CPU `method = \"ivfpq_fastscan\"` currently ",
                "supports `metric = \"euclidean\"`, ",
                "`\"cosine\"`, `\"correlation\"`, or ",
                "`\"inner_product\"`.",
                call. = FALSE
        ...
        switch(
            ...
            cagra = stop(
                "`method = \"cagra\"` is only available with ",
                "`backend = \"cuda\"`.",
                call. = FALSE
            ),
        ...
        if (
            ...
            stop(
                "CUDA `method = \"ivfpq_fastscan\"` currently ",
                "supports `metric = \"euclidean\"`, ",
                "`\"cosine\"`, `\"correlation\"`, or ",
                "`\"inner_product\"`.",
                call. = FALSE
        ...
        if (
            identical(metric, "inner_product") && identical(method, "nndescent")
        ) {
            stop(
                "CUDA `method = \"nndescent\"` does not ",
                "support raw inner product because ",
        ...
        if (
            ...
            stop(
                "CUDA `metric = \"inner_product\"` currently ",
                "supports `method = \"exact\"`, ",
                "`\"bruteforce\"`, `\"flat\"`, `\"ivf\"`, ",
                "`\"ivfpq\"`, `\"ivfpq_fastscan\"`, ",
                "`\"hnsw\"`, `\"nsg\"`, `\"vamana\"`, or ",
                "`\"cagra\"`.",
                call. = FALSE
            )
        ...
        switch(
            method,
            exact = if (isTRUE(faiss_gpu_available())) {
                "faiss_gpu_flat_l2"
            } else if (isTRUE(cuvs_available())) {
                "cuda_cuvs_bruteforce"
            } else {
                "cuda"
            },
            bruteforce = if (isTRUE(cuvs_available())) {
                "cuda_cuvs_bruteforce"
...
nn_resolved_backend_public_method <- function(backend) {
    ...
    if (backend %in% c("cuda_cuvs_bruteforce")) {
        return("bruteforce")
    }
    if (
        backend %in%
    ...
    if (backend %in% c("faiss_hnsw", "cuda_cuvs_hnsw", "cuvs_hnsw")) {
        return("hnsw")
    }
    if (
        backend %in%
    ...
    if (backend %in% c("faiss_nsg", "cpu_nsg", "cuda_nsg")) {
        return("nsg")
    }
    if (
        backend %in%
...
public_nn_cuda_route_available <- function(
    ...
    switch(
        method,
        exact = isTRUE(faiss_gpu_available_value) ||
            isTRUE(cuvs_available_value) ||
            isTRUE(cuda_available_value),
        bruteforce = isTRUE(faiss_gpu_available_value) ||
            isTRUE(cuvs_available_value) ||
            isTRUE(cuda_available_value),
...
nn_auto_select_shape_cpp <- function(
    ...
    out <- nn_auto_select_backend_cpp(
        ...
        cuvs_bruteforce_work_threshold = numeric_option(
        ...
        target_recall_option = as.numeric(normalize_hnsw_target_recall(
            target_recall
        )),
        tuning = normalize_nn_tuning(tuning)
    )
...
nn_auto_hardware_metadata <- function(route) {
    ...
    calibration_model <- if (identical(device, "cuda")) {
        route$calibration_gpu_model %||% NA_character_
    } else {
        route$calibration_cpu_model %||% NA_character_
    }
    runtime_model <- if (identical(device, "cuda")) {
        nn_runtime_gpu_model()
    } else {
        nn_runtime_cpu_model()
    }
    ...
    route$hardware_evidence_note <- if (identical(match_status, "matched")) {
        paste0(
...
grid_bins_per_dim <- function(n, k, p) {
    ...
    if (is.null(value)) {
        value <- faissr_option("grid_bins_per_dim", NULL)
    }
    if (!is.null(value)) {
...
select_cpu_spatial_backend <- function(data, k, exclude_self = TRUE) {
    ...
    anisotropy_threshold <- as.numeric(faissr_option(
        "cpu_spatial_anisotropy_threshold",
        0.02
    ))
    if (!is.finite(anisotropy_threshold) || anisotropy_threshold <= 0) {
        anisotropy_threshold <- 0.02
    }
    ...
    sample_bins <- as.integer(faissr_option(
        "cpu_spatial_sample_bins",
        if (p == 3L) 16L else 32L
    ))
    if (!is.finite(sample_bins) || is.na(sample_bins) || sample_bins < 4L) {
        sample_bins <- if (p == 3L) 16L else 32L
    }
    ...
    if (p == 3L) {
        cell <- cell + sample_bins * sample_bins * coords[[3L]]
    }
    counts <- tabulate(cell + 1L, nbins = sample_bins^p)
    mean_count <- sample_n / length(counts)
    imbalance <- max(counts) / max(mean_count, 1e-12)
    imbalance_threshold <- as.numeric(faissr_option(
        "cpu_spatial_imbalance_threshold",
        20
    ))
    if (!is.finite(imbalance_threshold) || imbalance_threshold <= 0) {
...
normalized_euclidean_metric_inputs <- function(
    ...
    if (identical(storage, "float")) {
        data_metric <- normalized_float32_transform_cached(
            data,
            metric,
            role = "data"
        )
        if (!is.null(data_metric)) {
            points_metric <- if (isTRUE(self_query)) {
                ...
                normalized_float32_transform_cached(
                    points,
                    metric,
                    role = "points"
                )
            ...
            return(list(
                ...
                transform_cache = list(
                    ...
                    points_hit = if (isTRUE(self_query)) {
                        isTRUE(data_metric$cache_hit)
                    } else {
                        isTRUE(points_metric$cache_hit)
                    },
                    data_key = data_metric$cache_key,
                    points_key = if (isTRUE(self_query)) {
                        data_metric$cache_key
                    } else {
                        points_metric$cache_key
                    },
                    row_major = TRUE
                )
            ))
...
faiss_flat_normalized_metric_result <- function(
    ...
    exact_params <- if (is.null(accelerator)) {
        cpu_flatlike_params(
            data_dim[[1L]],
            data_dim[[2L]],
            k,
    ...
    if (is.null(accelerator) && (any(data_zero) || any(points_zero))) {
        ...
        if (!is.null(accelerator)) {
            faiss_meta$accelerator <- accelerator
        }
    ...
    out <- with_faiss_query_batch_size(exact_params %||% list(), {
        if (isTRUE(use_float32_transform) && identical(accelerator, "cuda")) {
            nn_faiss_gpu_flat_float32_cpp(
                data_metric,
                points_metric,
                as.integer(k),
...
faiss_ivf_normalized_metric_result <- function(
    ...
    out <- if (
        isTRUE(use_float32_transform) && identical(accelerator, "cuda")
    ) {
        nn_faiss_gpu_ivf_flat_float32_cpp(
            data_metric,
            points_metric,
    ...
    if (is.null(accelerator)) {
        attr(result, "approximation")$accelerator <- NULL
    }
    result <- append_nn_tuning_metadata(result, params)
    if (isTRUE(use_float32_transform)) {
        result <- finish_float32_direct_result(result, out)
...
faiss_ivfpq_normalized_metric_result <- function(
    ...
    if (is.null(attr(result, "approximation")$role)) {
        attr(result, "approximation")$role <- NULL
    }
    if (is.null(attr(result, "approximation")$default_candidate)) {
        attr(result, "approximation")$default_candidate <- NULL
    }
    result <- append_nn_tuning_metadata(
        result,
        params,
...
faiss_ivfpq_fastscan_normalized_metric_result <- function(
    ...
    if (!is.null(cached)) {
        ...
        approximation$strategy <- .fastscan_normalized_strategy
        approximation$metric <- metric
        approximation$transform <- if (identical(metric, "correlation")) {
            "row_center_l2_normalize_then_IndexIVFPQFastScan_METRIC_L2"
        } else {
            "row_l2_normalize_then_IndexIVFPQFastScan_METRIC_L2"
        }
        approximation$metric_transform <- metric_inputs$transform
        approximation$distance_transform <-
            .normalized_similarity_distance_transform
        approximation$transform_storage <- metric_inputs$transform_storage %||%
            "double"
        approximation$transform_cache <- metric_inputs$transform_cache %||% NULL
        attr(result, "approximation") <- approximation
        result <- append_nn_tuning_metadata(
            result,
            params$ivf,
    ...
    attr(result, "approximation") <- list(
        strategy = .fastscan_normalized_strategy,
        backend = "faiss_ivfpq_fastscan",
        library = "faiss",
        accelerator = NULL,
        metric = metric,
        ...
        metric_transform = metric_inputs$transform,
...
vamana_params <- function(
    ...
    out <- nn_tune_vamana_cpp(
        ...
        )),
        {
            ...
            backend_value <- faissr_option(
                paste0(option_prefix, "_alpha"),
                NULL
            )
            if (!is.null(backend_value)) {
                nn_option_double_or_na(paste0(option_prefix, "_alpha"))
...
vamana_self_knn <- function(
    ...
    if (!is.null(ann_seed)) {
        ...
        seed_backend <- paste0(
            "native_",
            attr(ann_seed, "seed_backend", exact = TRUE),
            "_seed"
        )
    } else if (
        isTRUE(use_cuda) &&
        ...
        seed_backend_name <- if (identical(metric, "inner_product")) {
            "faiss_flat_ip"
        } else {
            "faiss_flat_l2"
        }
...
faiss_self_knn <- function(
    ...
    if (
        identical(backend, "faiss_nndescent") ||
    ...
    params <- faiss_ivf_params(n, k, metric = metric, p = ncol(data))
...
cpu_nndescent_params <- function(
    ...
    requested_max_candidates <- nn_option_int_or_na(
        "cpu_nndescent_max_candidates"
    )
    requested_n_random_projections <- nn_option_int_or_na(
        "cpu_nndescent_n_random_projections"
    )
    ...
    tune$requested_pool_size <- if (is.na(requested_pool_size)) {
        as.integer(tune$pool_size)
    } else {
        as.integer(requested_pool_size)
    }
    tune$requested_n_iters <- if (is.na(requested_n_iters)) {
        as.integer(tune$n_iters)
    } else {
        as.integer(requested_n_iters)
    }
    tune$requested_max_candidates <- if (is.na(requested_max_candidates)) {
        as.integer(tune$max_candidates)
    } else {
        as.integer(requested_max_candidates)
    }
    tune$requested_n_random_projections <- if (
        is.na(requested_n_random_projections)
    ) {
        as.integer(tune$n_random_projections)
    } else {
        as.integer(requested_n_random_projections)
    }
    tune
}
...
nndescent_self_knn <- function(
    ...
    if (is.null(n_threads)) {
        ...
        if (
            length(n_threads) != 1L || is.na(n_threads) || !is.finite(n_threads)
        ) {
            n_threads <- 1L
        }
    }
...
cuvs_ivfpq_align_params <- function(pq, p) {
    ...
    if (!aligned(pq_dim, pq_bits)) {
        ...
        effective_dim <- as.integer(max(
            1L,
            min(p, if (pq_dim > 0L) pq_dim else p)
        ))
...
ivfpq_fastscan_cpu_params <- function(
    ...
    pq <- faiss_ivfpq_pq_params(p, n = n, ivf_params = ivf)
    ...
    default_refine_factor <- ivfpq_fastscan_default_int(
        ivf$ivfpq_fastscan_refine_factor,
        8L
    )
    ...
    list(
        ...
        tuning = list(
            ...
            tuning_rule = ivf$tuning_rule %||% "hpc_cpu_ivfpq_fastscan",
            tuning_metric = ivf$tuning_metric %||% "euclidean",
            tuning_backend = ivf$tuning_backend %||% "cpu",
...
ivfpq_fastscan_cuda_params <- function(
    ...
    batch_size <- faissr_quiet_warning(as.integer(
        ivf$cuvs_ivf_batch_size %||% NA_integer_
    ))
    if (
    ...
    list(
        ...
        tuning = list(
            tuning_policy = "auto_shape_k_target_recall_ivfpq_fastscan",
            tuning_rule = ivf$tuning_rule %||% "cuda_cuvs_ivfpq_fastscan_4bit",
            tuning_metric = metric,
            tuning_backend = "cuda",
            tuning_method = "ivfpq_fastscan",
            tuning_shape_group = ivf$tuning_shape_group %||% NA_character_,
            tuning_k_bucket = ivf$tuning_k_bucket %||% NA_integer_,
            tuning_target_recall_code = ivf$tuning_target_recall_code %||%
                NA_integer_,
            ...
            tuning_benchmark_source = ivf$tuning_benchmark_source %||%
                NA_character_,
            tuning_source = ivf$tuning_source %||% "cpp",
            cuvs_ivf_batch_size = batch_size,
            ivfpq_fastscan = TRUE
        )
    )
}
...
faiss_gpu_ivf_tune_params <- function(data, k, base_params, tuning = "auto") {
    ...
    if (length(target) != 1L || is.na(target) || !is.finite(target)) {
        target <- 0.985
    }
    ...
    if (inherits(reference, "error")) {
        ...
        faiss_gpu_ivf_set_cached_tuning(
            key,
            out,
            persist = !identical(policy, "pilot")
        )
        return(out)
    }
    ...
    if (nrow(success) < 1L) {
        ...
        eligible <- success[
            is.finite(success$recall) & success$recall >= target,
            ,
            drop = FALSE
        ]
        if (nrow(eligible) > 0L) {
            chosen_row <- eligible[
                order(eligible$seconds, -eligible$recall),
                ,
                drop = FALSE
            ][1L, , drop = FALSE]
            status <- "target_met"
            ...
            chosen_row <- success[
                order(-success$recall, success$seconds),
                ,
                drop = FALSE
            ][1L, , drop = FALSE]
...
cuvs_cagra_tune_params <- function(
    ...
    if (nrow(success) < 1L) {
        ...
        chosen <- list(
            ...
            intermediate_graph_degree = as.integer(
                chosen_row$intermediate_graph_degree
            ),
...
nn <- function(
    ...
    requested_method_input <- trimws(as.character(method)[1L])
    ...
    result$requested_method_input <- requested_method_input
    attr(result, "requested_method_input") <- requested_method_input
    attr(result, "tuning") <- tuning
    attr(result, "target_recall") <- target_recall
    result$exclude_self <- isTRUE(exclude_self)
    attr(result, "exclude_self") <- result$exclude_self
...
nn_gpu <- function(
    ...
    if (identical(method, "auto")) {
        ...
        auto_preferred_backend <- nn_auto_selected_backend(
            auto_selection,
            "faiss_gpu_flat_l2"
        )
        auto_preferred_method <- nn_resolved_backend_public_method(
            auto_preferred_backend
        ) %||%
            "exact"
        if (
            length(auto_preferred_method) != 1L ||
        ...
        if (auto_preferred_method %in% c("exact", "flat", "bruteforce")) {
            ...
            auto_residency_constraint <- .gpu_exact_residency_constraint
    ...
    if (!inherits(out, "faissR_gpu_knn")) {
        ...
        has_gpu_contract <- is.list(out) &&
            all(gpu_contract_fields %in% names(out)) &&
            identical(out$result_residency, "cuda") &&
            typeof(out$handle) == "externalptr" &&
            typeof(out$indices_ptr) == "externalptr" &&
    ...
    out$exclude_self <- isTRUE(exclude_self)
    ...
    attr(out, "exclude_self") <- isTRUE(exclude_self)
    attr(out, "gpu_resident_execution_method") <- resolved_method
    attr(out, "gpu_resident_execution_backend") <- out$backend_used %||%
        NA_character_
    if (is.list(execution_tuning)) {
    ...
    attach_nn_distance_contract(out, metric)
}
