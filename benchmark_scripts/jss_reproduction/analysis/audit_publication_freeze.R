...
main <- function() {
  ...
  required <- c(
    ...
    "acquisition_date", "preprocessing", "source_file"
  )
  ...
  expected_datasets <- unique(as.character(manifest$dataset))
  missing_provenance <- setdiff(expected_datasets, as.character(provenance$dataset))
  duplicate_provenance <- unique(provenance$dataset[duplicated(provenance$dataset)])
  provenance$redistribution_permitted <- tolower(trimws(as.character(
    provenance$redistribution_permitted
  )))
  provenance$redistribution_resolved <- provenance$redistribution_permitted %in% c("yes", "no")
  if (!"dataset_md5" %in% names(provenance)) provenance$dataset_md5 <- ""
  if (!"dataset_sha256" %in% names(provenance)) provenance$dataset_sha256 <- ""
  provenance$dataset_md5 <- trimws(as.character(provenance$dataset_md5))
  provenance$dataset_sha256 <- trimws(as.character(provenance$dataset_sha256))
  provenance$fingerprint_present <-
    grepl("^[[:xdigit:]]{32}$", provenance$dataset_md5) |
    grepl("^[[:xdigit:]]{64}$", provenance$dataset_sha256)
  manifest_index <- match(provenance$dataset, manifest$dataset)
  manifest_md5 <- if ("dataset_md5" %in% names(manifest)) {
    as.character(manifest$dataset_md5[manifest_index])
  } else {
    rep(NA_character_, nrow(provenance))
  }
  provenance$fingerprint_matches_manifest <-
    is.na(manifest_index) |
    !nzchar(provenance$dataset_md5) |
    is.na(manifest_md5) |
    !nzchar(manifest_md5) |
    tolower(provenance$dataset_md5) == tolower(manifest_md5)
  ...
  }) & provenance$redistribution_resolved & provenance$fingerprint_present &
    provenance$fingerprint_matches_manifest
  ...
  problems <- c(
    ...
    if (length(missing_provenance)) paste("missing provenance for", paste(missing_provenance, collapse = ", ")) else NULL,
    if (length(duplicate_provenance)) paste("duplicate provenance for", paste(duplicate_provenance, collapse = ", ")) else NULL,
    if (any(!provenance$redistribution_resolved)) paste(sum(!provenance$redistribution_resolved), "unresolved redistribution rows") else NULL,
    if (any(!provenance$fingerprint_present)) paste(sum(!provenance$fingerprint_present), "provenance rows without a valid MD5 or SHA-256 fingerprint") else NULL,
    if (any(!provenance$fingerprint_matches_manifest)) paste(sum(!provenance$fingerprint_matches_manifest), "provenance fingerprints differing from the frozen manifest") else NULL,
