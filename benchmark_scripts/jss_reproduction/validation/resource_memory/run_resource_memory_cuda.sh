...
#SBATCH --job-name="frJ_mem_gpu"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/frJ_mem_gpu_%A_%a.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/frJ_mem_gpu_%A_%a.err
...
OUT="${BASE_DIR}/faissR_JSS_REPRODUCTION/validation/resource_memory/cuda_${SLURM_ARRAY_JOB_ID}/task_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${BASE_DIR}/benchmark_logs" "${OUT}"
singularity exec --nv --bind "${BASE_DIR}:${BASE_DIR}" "${SINGULARITY_IMAGE}" \
  Rscript -e 'stopifnot(identical(as.character(packageVersion("faissR")), Sys.getenv("EXPECTED_FAISSR_VERSION")), faissR::cuda_available())'
singularity exec --nv --bind "${BASE_DIR}:${BASE_DIR}" "${SINGULARITY_IMAGE}" \
  Rscript "${HERE}/benchmark_resource_memory.R" \
  --manifest="${BASE_DIR}/Data/float32_dataset_manifest_jmlr.csv" \
  --dataset="${dataset}" --backend=cuda --method="${method}" \
  --metric=euclidean --k=30 --target_recall=0.99 --query_mode="${mode}" \
  --repeat_id="${repeat_id}" --threads=2 --timeout=12000 \
  --out_dir="${OUT}"
