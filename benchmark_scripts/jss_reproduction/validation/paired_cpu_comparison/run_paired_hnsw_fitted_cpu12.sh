...
#SBATCH --job-name="frJ_pair_fit"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/frJ_pair_fit_%A_%a.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/frJ_pair_fit_%A_%a.err
...
OUT_DIR="${BASE_DIR}/faissR_JSS_REPRODUCTION/validation/paired_cpu_fitted/${DATASET}/k${K}/${COMPARATOR}_${SLURM_ARRAY_JOB_ID}_${TASK_ID}"
mkdir -p "${OUT_DIR}"
...
singularity exec --bind "${BASE_DIR}:${BASE_DIR}" "${SINGULARITY_IMAGE}" \
  ...
  --reference_k=100 --target_recall=0.99 --phases=fitted
