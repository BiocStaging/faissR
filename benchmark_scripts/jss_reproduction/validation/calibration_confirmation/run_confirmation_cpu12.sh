...
#SBATCH --job-name="frJ_conf_cpu"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/frJ_conf_cpu_%A_%a.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/frJ_conf_cpu_%A_%a.err
...
MANIFEST="${BASE_DIR}/faissR_JSS_REPRODUCTION/validation/calibration_confirmation/manifests/confirmation_cpu.csv"
OUT="${BASE_DIR}/faissR_JSS_REPRODUCTION/validation/calibration_confirmation/runs/${CONFIRMATION_ID}/cpu/cell_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${BASE_DIR}/benchmark_logs" "${OUT}"

singularity exec --bind "${BASE_DIR}:${BASE_DIR}" "${SINGULARITY_IMAGE}" \
  Rscript -e 'actual <- as.character(packageVersion("faissR")); expected <- Sys.getenv("EXPECTED_FAISSR_VERSION"); if (!identical(actual, expected)) stop("Expected ", expected, "; found ", actual)'
