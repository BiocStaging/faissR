...
parse_args <- function(x) {
  out <- list(
    phase = "list", dry_run = FALSE, backend = "all",
    start = 1L, limit = Inf
  )
  i <- 1L
  while (i <= length(x)) {
    if (identical(x[[i]], "--dry-run")) {
      ...
      i <- i + 1L
    } else if (identical(x[[i]], "--backend") && i < length(x)) {
      out$backend <- x[[i + 1L]]
      i <- i + 2L
    } else if (startsWith(x[[i]], "--backend=")) {
      out$backend <- sub("^--backend=", "", x[[i]])
      i <- i + 1L
    } else if (identical(x[[i]], "--start") && i < length(x)) {
      out$start <- as.integer(x[[i + 1L]])
      i <- i + 2L
    } else if (startsWith(x[[i]], "--start=")) {
      out$start <- as.integer(sub("^--start=", "", x[[i]]))
      i <- i + 1L
    } else if (identical(x[[i]], "--limit") && i < length(x)) {
      out$limit <- as.integer(x[[i + 1L]])
      i <- i + 2L
    } else if (startsWith(x[[i]], "--limit=")) {
      out$limit <- as.integer(sub("^--limit=", "", x[[i]]))
      i <- i + 1L
    } else {
      stop("Unknown argument: ", x[[i]], call. = FALSE)
    }
  }
  if (!out$backend %in% c("all", "cpu", "cuda")) {
    stop("`--backend` must be all, cpu, or cuda.", call. = FALSE)
  }
  if (is.na(out$start) || out$start < 1L) {
    stop("`--start` must be a positive integer.", call. = FALSE)
  }
  if (!is.infinite(out$limit) && (is.na(out$limit) || out$limit < 1L)) {
...
phase_contract <- function() {
  data.frame(
    ...
    expected_jobs = c(2L, 4L, 63L, 1L, 113L, 14L, 4L, 2L),
    proceed_when = c(
...
submit_phase <- function(
    ...
    backend = "all", start = 1L, limit = Inf) {
  ...
  if (!backend %in% c("all", "cpu", "cuda")) {
    stop("`backend` must be all, cpu, or cuda.", call. = FALSE)
  }
  if (length(start) != 1L || is.na(start) || start < 1L) {
    stop("`start` must be a positive integer.", call. = FALSE)
  }
  if (length(limit) != 1L ||
      (!is.infinite(limit) && (is.na(limit) || limit < 1L))) {
    stop("`limit` must be a positive integer or Inf.", call. = FALSE)
  }
  if (!identical(backend, "all")) {
    marker <- paste0(.Platform$file.sep, backend, .Platform$file.sep)
    launchers <- launchers[grepl(marker, launchers, fixed = TRUE)]
  }
  available <- length(launchers)
  if (start > available) {
    stop(
      "`start` is ", start, " but only ", available,
      " launchers remain after backend filtering.", call. = FALSE
    )
  }
  last <- if (is.infinite(limit)) available else min(available, start + limit - 1L)
  launchers <- launchers[seq.int(start, last)]

  base_dir <- Sys.getenv("BASE_DIR", unset = "/scratch/firenze/NN")
  image <- Sys.getenv(
    "SINGULARITY_IMAGE",
    unset = file.path(
      base_dir, "singularity", "fastembedr_cuda_faissR_0.99.21.sif"
    )
  )
  expected_version <- Sys.getenv(
    "EXPECTED_FAISSR_VERSION", unset = "0.99.21"
  )
  ...
  if (dry_run) {
    ...
    Sys.setenv(
      ...
      SINGULARITYENV_FAISSR_IMAGE_COMMIT = commit,
      APPTAINERENV_FAISSR_IMAGE_COMMIT = commit
    )
  }
  ...
  log_path <- file.path(
    log_dir,
    sprintf(
      "%s_%s_start%s_limit%s_%s_pid%s.csv", phase, backend, start,
      if (is.infinite(limit)) "all" else limit,
...
if (!identical(Sys.getenv("FAISSR_JSS_SOURCE_ONLY"), "true")) {
  ...
  submit_phase(
    args$phase, args$dry_run, backend = args$backend,
