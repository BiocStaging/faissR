...
stochastic_external_methods <- function() {
  c(
    ...
    "RcppAnnoy_euclidean", "RcppAnnoy_angular", "RcppHNSW_hnsw",
...
quality_metrics <- function(obj, ref, rows, k, exact_atol = 1e-5,
  ...
  empty <- data.frame(
    ...
    tie_aware_exact_atol = exact_atol,
    tie_aware_exact_rtol = exact_rtol,
  ...
  tie_aware_query_pass <- logical(length(rows))
  tie_aware_max_errors <- rep(Inf, length(rows))
  ...
  for (ii in seq_along(rows)) {
    ...
    sorted_candidate <- sort(as.numeric(candidate_distances))
    sorted_reference <- sort(as.numeric(reference_distances))
    finite_pair <- is.finite(sorted_candidate) & is.finite(sorted_reference)
    distance_equivalent <- FALSE
    if (length(sorted_candidate) == length(sorted_reference) &&
        all(finite_pair)) {
      exact_error <- abs(sorted_candidate - sorted_reference)
      exact_tolerance <- exact_atol + exact_rtol * pmax(
        abs(sorted_candidate), abs(sorted_reference)
      )
      distance_equivalent <- all(exact_error <= exact_tolerance)
      tie_aware_max_errors[[ii]] <- max(exact_error)
    }
    common <- intersect(
    ...
    tie_aware_query_pass[[ii]] <- is.finite(recalls[[ii]]) &&
      (recalls[[ii]] == 1 || distance_equivalent)
  ...
  data.frame(
    ...
    tie_aware_exact_max_distance_error = if (any(is.finite(tie_aware_max_errors)))
      max(tie_aware_max_errors[is.finite(tie_aware_max_errors)]) else NA_real_,
    tie_aware_exact_atol = exact_atol,
    tie_aware_exact_rtol = exact_rtol,
...
method_thread_metadata <- function(method, threads) {
  ...
  if (id %in% c("RcppAnnoy_euclidean", "RcppAnnoy_angular")) {
    return(list(
      requested = as.integer(threads),
...
aggregate_success_rows <- function(success) {
  ...
  pieces <- lapply(split(success, key), function(x) {
    ...
    memory_valid <- if ("memory_valid_for_comparison" %in% names(x)) {
      value <- suppressWarnings(as.logical(x$memory_valid_for_comparison))
      !is.na(value) & value
    } else {
      rep(FALSE, nrow(x))
    }
    rss <- suppressWarnings(as.numeric(x$peak_rss_gb[memory_valid]))
    gpu_memory <- if ("gpu_memory_peak_mib" %in% names(x)) {
      suppressWarnings(as.numeric(x$gpu_memory_peak_mib))
    } else {
      numeric()
    }
    exact_values <- if ("exact" %in% names(x)) {
      suppressWarnings(as.logical(x$exact))
    } else {
      rep(NA, nrow(x))
    }
    exact_family <- all(
      tolower(as.character(x$public_method)) %in%
        c("exact", "flat", "bruteforce", "grid") |
        grepl("_(exact|flat|bruteforce|grid)$", tolower(x$method_id)) |
        (!is.na(exact_values) & exact_values)
    )
    tie_values <- if ("tie_aware_exact_pass" %in% names(x)) {
      suppressWarnings(as.logical(x$tie_aware_exact_pass))
    } else {
      rep(NA, nrow(x))
    }
...
target_aware_external_comparison <- function(aggregate,
  ...
  if (!"selection_eligible" %in% names(faissr)) {
    exact_family <- tolower(as.character(faissr$method_id)) %in%
      c("exact", "flat", "bruteforce", "grid") |
      grepl("_(exact|flat|bruteforce|grid)$", tolower(faissr$method_id))
    faissr$selection_eligible <- exact_family |
      (!is.na(faissr$target_met_all_runs) & faissr$target_met_all_runs)
  }
...
worker_main <- function(args) {
  ...
  base <- data.frame(
    ...
    memory_measurement_source = "linux_proc_self_status_VmHWM",
    memory_scope = "shared_worker_process_lifetime_high_water",
    memory_process_isolated = FALSE,
    memory_valid_for_comparison = FALSE,
  ...
  tryCatch({
    ...
    base$tie_aware_exact_atol <- q$tie_aware_exact_atol
    base$tie_aware_exact_rtol <- q$tie_aware_exact_rtol
...
summarize_results <- function(out_dir, methods, config) {
  ...
  if (nrow(success)) {
    ...
    eligible <- is.na(aggregate$target_recall) | aggregate$selection_eligible
  ...
  md <- c(
    ...
    "- `jmlr_repeated_run_summary.csv`: medians, timing IQRs, replicate mean-query recall, and all-replicate target attainment.",
    "- `jmlr_best_robust_by_dataset_backend_metric_k_target.csv`: fastest method only after every prespecified replicate's mean query-level recall@k meets the target.",
