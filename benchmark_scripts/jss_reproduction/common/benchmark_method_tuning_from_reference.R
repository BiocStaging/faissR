...
recall_summary <- function(actual_indices, reference_indices) {
  if (is.null(reference_indices) || !length(reference_indices)) {
    return(list(recall_at_k = NA_real_, mean_query_recall_at_k = NA_real_,
  ...
  mean_recall <- if (length(vals)) mean(vals) else NA_real_
  median_recall <- if (length(vals)) stats::median(vals) else NA_real_
  min_recall <- if (length(vals)) min(vals) else NA_real_
  list(
    recall_at_k = mean_recall,
    mean_query_recall_at_k = mean_recall,
    median_recall_at_k = median_recall,
    median_query_recall_at_k = median_recall,
    min_recall_at_k = min_recall,
    min_query_recall_at_k = min_recall
  )
}
...
run_method <- function(config) {
  ...
  row$mean_query_recall_at_k <- quality$mean_query_recall_at_k
