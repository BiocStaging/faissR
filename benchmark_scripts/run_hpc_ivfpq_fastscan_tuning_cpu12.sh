...
#SBATCH --job-name="faissR_IVFPQ_FASTSCAN_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_ivfpq_fastscan_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_ivfpq_fastscan_cpu12_%j.err

set -euo pipefail


# CPU-only faissR IVFPQ_FASTSCAN tuning benchmark using precomputed exact references.
#
...
BENCH_SCRIPT="$(resolve_script benchmark_ivfpq_fastscan_tuning_from_reference.R)"
...

{
  ...
  [[ -n "${IVFPQ_FASTSCAN_PQ_M_VALUES}" ]] && BENCH_ARGS+=(--ivfpq_fastscan_pq_m_values="${IVFPQ_FASTSCAN_PQ_M_VALUES}")
  [[ -n "${IVFPQ_FASTSCAN_BBS_VALUES}" ]] && BENCH_ARGS+=(--ivfpq_fastscan_bbs_values="${IVFPQ_FASTSCAN_BBS_VALUES}")
