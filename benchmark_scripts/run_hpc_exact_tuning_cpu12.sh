...
#SBATCH --job-name="faissR_EXACT_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_exact_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_exact_cpu12_%j.err

set -euo pipefail


# CPU-only faissR EXACT tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_exact_tuning_cpu12.sh
#
# The job scans float32 .RData files, uses explicit backend="cpu",
# method="exact", the metrics listed in METRICS, k=15,30,50,100, and writes
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_EXACT_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_exact_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only EXACT tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/exact_tuning_cpu12.log"
