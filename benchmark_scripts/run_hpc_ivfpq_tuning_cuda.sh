...
#SBATCH --job-name="faissR_IVFPQ_CUDA"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_ivfpq_cuda_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_ivfpq_cuda_%j.err

set -euo pipefail


# CUDA faissR IVFPQ tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_ivfpq_tuning_cuda.sh
#
# Or submit a metric-specific wrapper such as:
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_ivfpq_tuning_cuda_correlation.sh
#
# The job scans float32 .RData files, uses explicit backend="cuda",
# method="ivfpq", the metrics listed in METRICS, k=15,30,50,100, and
# writes recommendation tables for target recall 0.90, 0.95, and 0.99.
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_IVFPQ_TUNING_CUDA_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_ivfpq_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CUDA IVFPQ tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/ivfpq_tuning_cuda.log"
