...
#SBATCH --job-name="faissR_IVF_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_ivf_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_ivf_cpu12_%j.err

set -euo pipefail


# CPU-only faissR IVF tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_ivf_tuning_cpu12.sh
#
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_IVF_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_ivf_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only IVF tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/ivf_tuning_cpu12.log"
