...
#SBATCH --job-name="faissR_FLAT_CUDA"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_flat_cuda_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_flat_cuda_%j.err

set -euo pipefail


# CUDA faissR FLAT tuning benchmark using precomputed exact references.
#
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_FLAT_TUNING_CUDA_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CUDA FLAT tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/flat_tuning_cuda.log"
