...
run_case <- function(x, backend, method, metric, k, threads, target_recall) {
  ...
  base <- data.frame(
    ...
    inner_product_rank_direction_pass = NA, degenerate_row_pass = NA,
  ...
  tryCatch({
    ...
    if (metric == "inner_product") {
      scores <- tcrossprod(x)
      rank_ok <- vapply(seq_len(nrow(x)), function(i) {
        selected <- indices[i, ]
        all(diff(scores[i, selected]) <= 1e-5)
      }, logical(1))
      base$inner_product_rank_direction_pass <- all(rank_ok)
    }
    ...
    invariant_ok <- metric == "inner_product" || !exact_family ||
      is.finite(base$invariant_recall) && base$invariant_recall >= 0.99
    ip_ok <- metric != "inner_product" || !exact_family ||
      isTRUE(base$inner_product_rank_direction_pass)
    base$metric_contract_pass <- base$dimensions_pass && base$finite_distance_pass &&
      base$sorted_distance_pass && base$route_pass && invariant_ok && ip_ok
...
main <- function() {
  ...
  metrics <- split_values(args$metrics, "euclidean,cosine,correlation,inner_product")
  ...
  writeLines(c(
    ...
    "The ordinary suite uses non-degenerate data and checks output dimensions, finite distances, ascending faissR distance order, resolved-device identity, exact-reference recall, Euclidean translation invariance, cosine positive-scale invariance, correlation positive-affine invariance, and raw-inner-product descending-score order for exact-family methods.",
    "",
    "For approximate methods, transform-invariance overlap and exact raw-score order are diagnostic rather than metric-contract requirements because quantization, graph search, and nondeterministic GPU training may reorder candidates. Their exact-reference recall and target attainment are reported separately.",
    "",
  ...
  if (any(bad_required) || any(bad_edge_required)) {
    if (any(bad_required)) {
      print(result[bad_required, c(
        ...
        "inner_product_rank_direction_pass", "route_pass", "error"
