...
```bash
for f in benchmark_scripts/jmlr_mloss_publication/cpu/*.sh; do echo "sbatch $f"; done
for f in benchmark_scripts/jmlr_mloss_publication/cuda/*.sh; do echo "sbatch $f"; done
```

Each faissR method tests Euclidean, cosine, correlation, and raw inner product;
...
```bash
sbatch benchmark_scripts/jmlr_mloss_publication/reviewer_response/run_metric_conformance_cpu12.sh
sbatch benchmark_scripts/jmlr_mloss_publication/reviewer_response/run_metric_conformance_cuda.sh
```
...
raw-inner-product score order, Euclidean translation invariance, cosine
...
```bash
sbatch benchmark_scripts/jmlr_mloss_publication/reviewer_response/run_selector_validation_cpu12.sh
sbatch benchmark_scripts/jmlr_mloss_publication/reviewer_response/run_selector_validation_cuda.sh
```
...
method, and performs leave-one-dataset-out sensitivity analysis. For a held-out
dataset, the analysis selects a method family using only other datasets in the
same predeclared shape group. If that group is absent, it uses the three nearest
datasets in log(n)-log(p) space. Coverage is maximized before median log runtime
is minimized. This analysis never edits or retrains the compiled selector.
...
`/scratch/firenze/NN/Data/dataset_provenance_jss.csv` and fill every field. Do
not guess licensing terms; use the authoritative dataset source and citation.
Then set the exact faissR commit embedded in the container and run both audits:

```bash
export FAISSR_PACKAGE_COMMIT=<40-character-faissR-git-commit>
sbatch --export=ALL benchmark_scripts/jmlr_mloss_publication/reviewer_response/run_freeze_audit_cpu12.sh
sbatch --export=ALL benchmark_scripts/jmlr_mloss_publication/reviewer_response/run_freeze_audit_cuda.sh
```
...
| Auto versus empirical oracle | `run_selector_validation_*.sh` and `../analysis/aggregate_publication_results.R` |
| Four-metric conformance | `run_metric_conformance_*.sh` |
...
| Speed-recall and auto-oracle figures | `../analysis/build_publication_figures.R` |
...
```text
/scratch/firenze/NN/faissR_JMLR_MLOSS/reviewer_response/
```
