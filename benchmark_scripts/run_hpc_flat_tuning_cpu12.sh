...
#SBATCH --job-name="faissR_FLAT_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_flat_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_flat_cpu12_%j.err

set -euo pipefail


# CPU-only faissR FLAT tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_flat_tuning_cpu12.sh
#
# The job scans float32 .RData files, uses explicit backend="cpu",
# method="flat", the metrics listed in METRICS, k=15,30,50,100, and writes
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_FLAT_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_flat_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only FLAT tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/flat_tuning_cpu12.log"
