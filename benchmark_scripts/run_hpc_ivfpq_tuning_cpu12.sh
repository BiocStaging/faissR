...
#SBATCH --job-name="faissR_IVFPQ_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_ivfpq_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_ivfpq_cpu12_%j.err

set -euo pipefail


# CPU-only faissR IVFPQ tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_ivfpq_tuning_cpu12.sh
#
# The job scans float32 .RData files, uses explicit backend="cpu",
# method="ivfpq", the metrics listed in METRICS, k=15,30,50,100, and writes
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_IVFPQ_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only IVFPQ tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/ivfpq_tuning_cpu12.log"
