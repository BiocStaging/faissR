...
result_row <- function(dataset, n, p, method, backend, centers, cycle, n_threads,
  ...
  data.frame(
    ...
    selection_explicit_backend = as.logical(selection_explicit_backend),
...
summarize_kmeans_cycles <- function(ok) {
  ...
  summary <- lapply(parts, function(x) {
    data.frame(
      ...
      selection_explicit_backend = any(x$selection_explicit_backend %in% TRUE, na.rm = TRUE),
...
run_one <- function(x, labels, dataset_name, method, backend, centers,
  ...
  tryCatch({
    ...
    result_row(
      ...
      selection_explicit_backend = selection$explicit_backend %||% if (identical(method, "stats")) TRUE else NA,
