...
#SBATCH --job-name="faissR_JMLR_CUDA"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_jmlr_tuned_cuda_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_jmlr_tuned_cuda_%j.err
...
export INCLUDE_GPU_RESIDENT="${INCLUDE_GPU_RESIDENT:-TRUE}"
export BUILD_MANIFEST="${BUILD_MANIFEST:-TRUE}"
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_JMLR_TUNED_CUDA_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running JMLR tuned CUDA benchmark"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/jmlr_tuned_cuda.log"
