...
#SBATCH --job-name="faissR_CAGRA_CUDA"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_cagra_cuda_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_cagra_cuda_%j.err

set -euo pipefail


# CUDA faissR CAGRA tuning benchmark using precomputed exact references.
#
...
# method="cagra", METRICS, k=15,30,50,100, and writes recommendation
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_CAGRA_TUNING_CUDA_$(date +%Y%m%d_%H%M%S)}"
...
if [[ -z "${FAISSR_TIMEOUT_BIN:-}" ]]; then
  if [[ -x /usr/bin/timeout ]]; then
    export FAISSR_TIMEOUT_BIN="/usr/bin/timeout"
  elif command -v timeout >/dev/null 2>&1; then
    export FAISSR_TIMEOUT_BIN="$(command -v timeout)"
  else
    export FAISSR_TIMEOUT_BIN=""
  fi
fi
...
BENCH_SCRIPT="$(resolve_script benchmark_cagra_tuning_from_reference.R)"
