...
#SBATCH --job-name="faissR_VAMANA_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_vamana_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_vamana_cpu12_%j.err

set -euo pipefail


# CPU-only faissR VAMANA tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_vamana_tuning_cpu12.sh
#
# The job scans float32 .RData files, uses explicit backend="cpu",
# method="vamana", the comma-separated metrics in METRICS, k=15,30,50,100,
# and writes recommendation tables for target recall 0.90, 0.95, and 0.99.
# Metric-specific wrappers such as run_hpc_vamana_tuning_cpu12_cosine.sh
# set METRICS to a single distance.
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_VAMANA_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_vamana_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only VAMANA tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/vamana_tuning_cpu12.log"
