...
#SBATCH --job-name="faissR_NND_CPU12"
#SBATCH --chdir=/scratch/firenze/NN
#SBATCH --output=/scratch/firenze/NN/benchmark_logs/faissR_nndescent_cpu12_%j.out
#SBATCH --error=/scratch/firenze/NN/benchmark_logs/faissR_nndescent_cpu12_%j.err

set -euo pipefail


# CPU-only faissR NNDESCENT tuning benchmark using precomputed exact references.
#
...
#   sbatch /scratch/firenze/NN/benchmark_scripts/run_hpc_nndescent_tuning_cpu12.sh
#
# The job scans float32 .RData files, uses explicit backend="cpu",
# method="nndescent", the comma-separated metrics in METRICS, k=15,30,50,100,
# and writes recommendation tables for target recall 0.90, 0.95, and 0.99.
# Metric-specific wrappers such as run_hpc_nndescent_tuning_cpu12_cosine.sh
# set METRICS to one metric and save into a metric-specific output folder.
...
export OUT_DIR="${OUT_DIR:-${BASE_DIR}/faissR_NNDESCENT_TUNING_CPU12_$(date +%Y%m%d_%H%M%S)}"
...
BENCH_SCRIPT="$(resolve_script benchmark_nndescent_tuning_from_reference.R)"
...

{
  ...
  echo "[$(date --iso-8601=seconds)] running CPU-only NNDESCENT tuning from precomputed references"
  ...
  echo "DONE: ${OUT_DIR}"
} 2>&1 | tee -a "${OUT_DIR}/nndescent_tuning_cpu12.log"
