...
script_path <- function() {
  ...
  normalizePath(file.path("benchmark_scripts", "benchmark_exact_tuning_grid.R"), mustWork = FALSE)
}
...
clean_bool_values <- function(values, default) {
  raw <- tolower(split_arg(values, default))
  raw <- raw[raw %in% c("true", "false", "1", "0", "yes", "no", "on", "off")]
  unique(vapply(raw, logical_arg, logical(1)))
}

normalize_cuda_provider <- function(provider) {
  key <- tolower(gsub("[[:space:]_-]+", "", as.character(provider)))
  aliases <- c(
    cudaexact = "cuda_exact",
    exact = "cuda_exact",
    autoexact = "cuda_exact",
    faiss = "faiss_gpu_flat",
    faissgpu = "faiss_gpu_flat",
    faissgpuflat = "faiss_gpu_flat",
    flat = "faiss_gpu_flat",
    cuvs = "cuvs_bruteforce",
    cuvsbruteforce = "cuvs_bruteforce",
    bruteforce = "cuvs_bruteforce"
  )
...
method_for_provider <- function(provider) {
  switch(provider,
    cpu_exact = "exact",
    cuda_exact = "exact",
    faiss_gpu_flat = "flat",
    cuvs_bruteforce = "bruteforce",
    stop("Unsupported exact provider: ", provider, call. = FALSE)
  )
}

cpu_candidates <- function(k, thread_values, output_values, cpu_batches, cache_modes) {
  grid <- expand.grid(
    n_threads = thread_values,
    output = output_values,
    faiss_query_batch_size = cpu_batches,
    cache_fitted_indexes = cache_modes,
    KEEP.OUT.ATTRS = FALSE,
    stringsAsFactors = FALSE
  )
  grid$device <- "cpu"
  grid$provider <- "cpu_exact"
  grid$public_backend <- "cpu"
  grid$public_method <- "exact"
  grid$faiss_gpu_query_batch_size <- NA_integer_
  grid$faiss_gpu_reuse_resources <- NA
  grid$candidate_id <- sprintf(
    "cpu_exact_t%d_b%d_%s_cache%s",
    grid$n_threads,
    grid$faiss_query_batch_size,
    grid$output,
    ifelse(grid$cache_fitted_indexes, "on", "off")
  )
...
cuda_candidates <- function(k, thread_values, output_values, providers,
                            gpu_batches, gpu_reuse_values) {
  rows <- list()
  idx <- 0L
  for (provider in providers) {
    if (identical(provider, "cuvs_bruteforce")) {
      grid <- expand.grid(
        n_threads = thread_values,
        output = output_values,
        KEEP.OUT.ATTRS = FALSE,
      ...
      grid <- expand.grid(
        n_threads = thread_values,
        output = output_values,
        faiss_gpu_query_batch_size = gpu_batches,
        faiss_gpu_reuse_resources = gpu_reuse_values,
        KEEP.OUT.ATTRS = FALSE,
        stringsAsFactors = FALSE
      )
      grid$faiss_query_batch_size <- NA_integer_
    ...
    grid$public_backend <- "cuda"
    grid$public_method <- method_for_provider(provider)
    grid$candidate_id <- if (identical(provider, "cuvs_bruteforce")) {
      sprintf("cuda_cuvs_bruteforce_t%d_%s", grid$n_threads, grid$output)
    } else {
      sprintf(
...
candidate_grid <- function(backend, k, thread_values, output_values,
                           cpu_batches, gpu_batches, cache_modes,
                           gpu_reuse_values, cuda_providers) {
  if (identical(backend, "cpu")) {
    return(cpu_candidates(k, thread_values, output_values, cpu_batches, cache_modes))
  }
  cuda_candidates(k, thread_values, output_values, cuda_providers, gpu_batches, gpu_reuse_values)
}
...
apply_candidate_runtime <- function(candidate) {
  ...
  if (!is.na(candidate$cache_fitted_indexes)) {
    options(
      faissR.cache_fitted_nn_indexes = isTRUE(candidate$cache_fitted_indexes),
      faissR.cache_fitted_nn_indexes_max_entries = 1L
    )
  }
  invisible(TRUE)
}
...
run_rscript_task <- function(task, config, timeout, bench_script) {
  config_path <- tempfile("faissR_exact_config_", fileext = ".rds")
  result_path <- tempfile("faissR_exact_result_", fileext = ".rds")
...
select_best_rows <- function(success, target_recalls, group_cols) {
  rows <- list()
  idx <- 0L
...
summarize_results <- function(out_dir, target_recalls) {
  results_path <- file.path(out_dir, "exact_tuning_results.csv")
  ...
  recommendations <- select_best_rows(success, target_recalls, c("dataset", "device", "k"))
  provider_recommendations <- select_best_rows(success, target_recalls, c("dataset", "device", "provider", "k"))
  write.csv(recommendations, file.path(out_dir, "exact_tuning_recommendations.csv"), row.names = FALSE)
  write.csv(provider_recommendations, file.path(out_dir, "exact_tuning_provider_recommendations.csv"), row.names = FALSE)
  ...
  if (nrow(success)) {
    ...
    write.csv(shape_candidates, file.path(out_dir, "exact_tuning_shape_candidates.csv"), row.names = FALSE)
    ...
    write.csv(shape_best, file.path(out_dir, "exact_tuning_shape_recommendations.csv"), row.names = FALSE)
  }
...
write_report <- function(out_dir, results, recommendations, provider_recommendations) {
  lines <- c(
    ...
    "## Tuned Parameters",
    "",
    "- CPU: OpenMP/R thread count, FAISS query batch size, output distance storage, fitted-index cache mode.",
    "- CUDA FAISS GPU Flat: FAISS GPU query batch size, GPU resource reuse flag, output distance storage.",
    "- CUDA cuVS brute force: provider route and output distance storage.",
    "",
    "## Status Counts",
    "",
    ...
    "## Output Files",
    "",
    "- `exact_tuning_candidate_grid.csv`",
    "- `exact_tuning_results.csv`",
    "- `exact_tuning_recommendations.csv`",
    "- `exact_tuning_provider_recommendations.csv`",
    "- `exact_tuning_shape_candidates.csv`",
    "- `exact_tuning_shape_recommendations.csv`"
  )
  ...
  writeLines(lines, file.path(out_dir, "exact_tuning_report.md"))
}
...
main <- function() {
  ...
  out_dir <- args$out_dir %||% file.path(getwd(), paste0("faissR_EXACT_TUNING_", toupper(backend), "_", format(Sys.time(), "%Y%m%d_%H%M%S")))
  ...
  cuda_providers <- normalize_cuda_provider(split_arg(args$cuda_providers, "cuda_exact,faiss_gpu_flat,cuvs_bruteforce"))
  if (backend == "cuda" && (any(is.na(cuda_providers)) || !length(cuda_providers))) {
    stop("`--cuda_providers` must contain `cuda_exact`, `faiss_gpu_flat`, `cuvs_bruteforce`, or aliases.", call. = FALSE)
  }
  cuda_providers <- unique(cuda_providers)
  ...
  write.csv(
    data.frame(
      ...
      value = c(backend, manifest_path, paste(datasets, collapse = ","),
                ...
                paste(cache_modes, collapse = ","), paste(gpu_reuse_values, collapse = ","),
                paste(cuda_providers, collapse = ","), reference_backend, max_working_gb)
    ),
    file.path(out_dir, "exact_tuning_config.csv"),
    row.names = FALSE
  )
