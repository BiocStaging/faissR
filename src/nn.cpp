...
double candidate_distance_row_major(const double* data,
                                    const double* points,
                                    const int data_row,
                                    const int point_row,
                                    const int n_features,
...
void write_candidate_knn_rows(const double* data,
  ...
  for (int q = query_start; q < query_end; ++q) {
    ...
    for (int c = 0; c < candidate_indices.ncol(); ++c) {
      ...
      if (one_based == NA_INTEGER || one_based < 1 || one_based > n_data) {
        continue;
      }
      ...
      const Neighbor item{
        candidate_distance_row_major(data, points, candidate, q, n_features, kind),
        candidate
      };
      if (static_cast<int>(top.size()) < k) {
        top.push_back(item);
...
// [[Rcpp::export]]
List candidate_knn_cpp(NumericMatrix data,
                       NumericMatrix points,
  ...
  if (n_features < 1 || points.ncol() != n_features) {
  ...
  auto worker = [&](const int start, const int end) {
    write_candidate_knn_rows(
      data_row_major.data(),
      points_row_major.data(),
      candidate_indices,
      n_data,
      n_points,
      n_features,
