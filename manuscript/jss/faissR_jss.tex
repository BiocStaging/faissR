...
\author{Moussa Kassim Mohamed$^{1,2,\dagger}$
...
\Plainauthor{Moussa Kassim Mohamed, Martin Ocharo, Dalia Ahmed, Dupe Ojo, Alessia Vignoli, Leonardo Tenori, Dinesh Gupta, Silvano Piazza, Stefano Cacciatore}
...
support through RAPIDS cuVS. A common interface supports exact and approximate
search with Euclidean, cosine, and correlation distances. The interface
accepts dense \proglang{R} double
...
established \proglang{R} interfaces. The results show that the preferred search
strategy depends on both the data and the workload: index construction affects
one-off searches, whereas reusable indexes can reduce the cost of repeated
queries. The package also provides an experimental calibration-based selector
for full self-search workloads. Reproducible examples and benchmark code
...
\Address{
  ...
  Trento, Italy\\[1ex]
  \textsuperscript{\(\dagger\)}Moussa Kassim Mohamed and Martin Ocharo contributed
...
These packages provide several choices for CPU search. \pkg{faissR} exposes
the FAISS index family and optional cuVS execution through a native
\proglang{R} interface. FAISS includes inverted-file (IVF),
...
builds \citep{cuVS,FaissCuVS}. Their data representations differ from those
typically used by \proglang{R}: R matrices are commonly double precision and
column-major, whereas FAISS and cuVS predominantly use row-major float32
arrays. \pkg{faissR} performs the required precision and memory-layout
conversion in compiled code, or accepts a \pkg{float} matrix directly where
supported \citep{float}. Neighbor results are returned in increasing order of
distance. For supported CPU methods, a fitted index can be retained and used
for later query matrices. CUDA results can remain in GPU memory until the user
requests a host copy or passes them to another compiled R package. Returned
metadata identify the implementation, conversion, and index-reuse status for
each call. This article evaluates the correctness, recall, and runtime of these
interfaces against existing R implementations. Automatic method selection is
evaluated separately as an optional experimental facility.
...
query matrices and report whether the requested CPU or CUDA device can proceed.
Search results record the distance definition and ordering in their metadata;
the field-level contract is given in the supplement and reference manual.
...
audit therefore accepts an alternative tied identifier when the sorted
distances agree within the fixed float32 tolerance specified in Supplementary
Section~\ref{supp-sec:supp-numerics}. That rule neither changes returned
distances nor determines approximate-route recall.
...
\citep{FuEtAl2019,SubramanyaEtAl2019,DongMosesLi2011}. They are package-owned
candidate-refinement procedures that retain selected pruning or update ideas
but omit the canonical index-construction and query-traversal algorithms,
hence the \code{\_style} suffix. On CUDA, \method{nndescent\_style} can instead
call the external cuVS implementation; returned metadata identify the route.
Supplementary Table~\ref{supp-tab:supp-derived-methods} and its pseudocode give
...
The experimental package-owned routes are excluded from the principal
performance comparisons. The low-dimensional grid method is checked
separately as an exact method; IVF-PQ and FastScan call their corresponding
...
The argument \code{backend} determines where the search is executed.
With \code{backend = "cpu"}, \pkg{faissR} uses an available host-processor
implementation, returns ordinary \proglang{R} matrices, and can retain fitted
indexes for compatible methods. With \code{backend = "cuda"}, it uses a
CUDA-enabled FAISS or cuVS implementation on an NVIDIA GPU. The function
\fct{nn} copies CUDA results to host matrices, whereas \fct{nn\_gpu} can keep
them in device memory. If \code{backend} is omitted, the package uses the
session option, then the \code{FAISSR\_BACKEND} environment variable, and
finally CPU. It does not choose automatically between CPU and CUDA. The
metadata field \code{backend\_used} records the concrete
implementation, such as \code{faiss\_hnsw}. Section~\ref{sec:tuning}
describes automatic method selection and parameter tuning.
...
Exact search supplies a reference result and may be competitive for a one-off
call. HNSW supports reusable CPU indexes; IVF separates index training from
search on large collections, and CAGRA provides CUDA graph search.
Table~\ref{tab:methods} summarizes the main controls and route status.
Experimental
...
Native indexes remain available while their owning R objects are alive.
Both host and CUDA identifiers are signed 32-bit integers, limiting the
interface to $2^{31}-1$ reference rows;
available memory and library limits usually impose smaller limits.
The supplement describes memory ownership for GPU-resident results.
...
\begin{CodeChunk}
\begin{CodeInput}
R> install.packages("/path/to/faissR_source.tar.gz",
...
\pkg{faissR} is Expat-licensed. It neither vendors nor redistributes FAISS
(Expat), cuVS (Apache-2.0), or the CUDA toolkit/driver (NVIDIA CUDA SDK terms).
...
The execution device is chosen explicitly with \code{backend = "cpu"} or
\code{backend = "cuda"}, or through the session configuration when the
argument is omitted. Two other arguments control separate decisions:
\code{method = "auto"} invokes the experimental method-family policy for the
chosen device, while \code{tuning = "auto"} selects parameters within the
resolved method. An unsupported request fails before execution.
The requested \code{target\_recall} is one of 0.90, 0.95, or 0.99.
The bundled automatic policy was developed for the cold self-search workload
defined below; its computing environment is reported in
Section~\ref{sec:computing-environment}.
...
Their dimensions, query counts, seeds, fingerprints, implementations, and
independent CPU checks are provided in the
...
CPU jobs ran on heterogeneous x86-64 compute nodes and requested 12 threads
per job. Matched CPU routes shared the same allocation and node. CUDA jobs used
one NVIDIA L40S with 46,068 mebibytes (MiB), compute capability 8.9, and driver
595.58.03. Both CPU and CUDA jobs used \proglang{R} 4.5.3 on Debian 13,
OpenBLAS 0.3.33, and a checksummed Singularity image \citep{Singularity}.
Supplementary
Table~\ref{supp-tab:supp-environment}
lists the software and hardware details.

The replication archive retains the package commit, library versions,
container digest, and data identifiers recorded for each experiment. Runtime
ratios compare calls within the same experiment.
...
implementation using a prespecified calibration sample and evaluated it on an
independent validation sample. It measured cold calls, direct index
construction, and fitted-index queries separately over five validation
repetitions.
...
and node; order was randomized. The complete dataset-level results appear in
Supplementary Table~\ref{supp-tab:supp-comprehensive-datasets} and
...
Table~\ref{supp-tab:supp-comprehensive-datasets} gives dataset-specific ratios
and eligible-pair counts. Per-dataset results for the fixed-configuration HNSW
experiment appear in Supplementary Table~\ref{supp-tab:supp-cpu-paired}.
...
\subsection{CUDA search and automatic method selection}

The installed experimental method policy selected exact-audited Flat in 280 of
...
\pkg{faissR} provides an R interface to FAISS and optional cuVS search for
dense input, reusable CPU indexes, and GPU-resident results. The measured
runtime varied by dataset and query workload. In the tuned HNSW experiment,
differences in index-construction time were larger than differences in queries
...
The optional automatic method policy remains experimental. Its CUDA calibration used
the cold self-search workload described in Section~\ref{sec:tuning}, while CPU
automatic method selection was not independently validated. The policy has no memory
budget or expected-reuse input. Shape descriptors capture size and dimension
but omit other aspects of data geometry, and one timing per calibration
configuration leaves selection sensitive to timing noise. The holdout analyses
cover the measured domains and computing environment. Exact search remains a
useful baseline for application-specific comparisons beyond that scope.

GPU-resident output exposes neighbor buffers to another compiled package
...
remain local to the R session. Returned metadata record the implementation,
input conversion, and index-reuse status for each result.
...
study (\url{https://ucthpc.uct.ac.za/}).
